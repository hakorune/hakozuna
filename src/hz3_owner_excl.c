...
static pthread_mutex_t g_owner_excl[HZ3_NUM_SHARDS];
static pthread_once_t g_owner_excl_once = PTHREAD_ONCE_INIT;

// TLS re-entry guard (depth counter per shard)
static __thread uint8_t t_owner_excl_depth[HZ3_NUM_SHARDS] = {0};
...
static void hz3_owner_excl_init_once(void) {
    for (int i = 0; i < HZ3_NUM_SHARDS; i++) {
        pthread_mutex_init(&g_owner_excl[i], NULL);
    }
}
...
Hz3OwnerExclToken hz3_owner_excl_acquire(uint8_t owner) {
    ...
    pthread_once(&g_owner_excl_once, hz3_owner_excl_init_once);
    pthread_mutex_lock(&g_owner_excl[owner]);
    token.locked = 1;
...
Hz3OwnerExclToken hz3_owner_excl_try_acquire(uint8_t owner) {
    ...
    pthread_once(&g_owner_excl_once, hz3_owner_excl_init_once);
    if (pthread_mutex_trylock(&g_owner_excl[owner]) != 0) {
        token.active = 0;
        return token;
    }
...
void hz3_owner_excl_release(Hz3OwnerExclToken token) {
    ...
    if (token.locked) {
        ...
        pthread_mutex_unlock(&g_owner_excl[token.owner]);
    }
}
