...
// Cached so_base (initialized once via pthread_once)
static pthread_once_t g_so_base_once = PTHREAD_ONCE_INIT;
...
static inline uintptr_t hz3_s86_get_so_base(void) {
    pthread_once(&g_so_base_once, hz3_s86_so_base_init_once);
    return g_so_base;
}
...
int hz3_central_shadow_record(void* ptr, void* next, void* ra) {
    ...
    pthread_once(&g_shadow_once, hz3_s86_shadow_init_once);
