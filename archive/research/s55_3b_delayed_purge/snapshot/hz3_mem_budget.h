...
// S55-3B: Process delayed purge queue (epoch boundary)
// - Checks queued runs for age (delay_epochs) and still-free status
// - Only madvise runs that remain free (discard reused runs)
// - current_epoch: t_repay_epoch_counter from caller (no global atomic++ here)
#if HZ3_S55_3B_DELAYED_PURGE
void hz3_delayed_purge_process(uint32_t current_epoch);
#else
static inline void hz3_delayed_purge_process(uint32_t current_epoch) { (void)current_epoch; }
#endif
