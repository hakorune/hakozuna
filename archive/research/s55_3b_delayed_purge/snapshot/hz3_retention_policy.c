...
// CRITICAL FIX: Use global atomic epoch counter (not TLS)
// - TLS epoch creates per-thread time axis → delayed purge age check fails
...
__thread uint64_t t_repay_epoch_counter_deprecated = 0;

// Exported for S55-3B delayed purge (global atomic, read-only)
// - NOTE: This is NOT t_repay_epoch_counter (which was TLS and broken)
uint64_t hz3_repay_epoch_get(void) {
    return atomic_load_explicit(&g_repay_epoch_counter, memory_order_relaxed);
}
...
void hz3_retention_repay_epoch(void) {
    ...
    if ((current_epoch % HZ3_S55_REPAY_EPOCH_INTERVAL) != 0) {
        // Early return, but epoch already incremented (delayed purge will see it)
        #if HZ3_S55_3B_DELAYED_PURGE
        hz3_delayed_purge_process((uint32_t)current_epoch);
        #endif
    ...
    // - Process: every L2 epoch (drain queue regardless of enqueue)
    // - Box Boundary: Clean separation (enqueue間引き、process毎回)
...
#endif
        // S55-3: Enqueue runs (S55-3B) or immediate madvise (S55-3)
        hz3_mem_budget_reclaim_medium_runs_subrelease(HZ3_S55_3_MEDIUM_SUBRELEASE_BUDGET_PAGES);
    }

    // S55-3B: Process delayed purge queue (every L2 epoch, not just on enqueue)
    // - CRITICAL: Process even when enqueue is skipped (drain queued runs)
    // - Pass current_epoch (global atomic, authoritative timeline)
#if HZ3_S55_3B_DELAYED_PURGE
    hz3_delayed_purge_process((uint32_t)current_epoch);
#endif
#endif
