...
static int hz10_shim_orphan_registry_drain_probe;
...
static int hz10_shim_orphan_registry_drain_probe_enabled(void) {
  const char* value = getenv("HZ10_SHIM_ORPHAN_REGISTRY_DRAIN_PROBE");
  return value && value[0] == '1' && value[1] == '\0';
}
 ...
 * - HZ10_SHIM_ORPHAN_REGISTRY_DRAIN_PROBE=1 drains orphan-registry remote
 *   stripes at process exit under a probe owner and reports drain potential.
...
__attribute__((constructor)) static void hz10_shim_init(void) {
  ...
  hz10_shim_orphan_registry_drain_probe =
      hz10_shim_orphan_registry_drain_probe_enabled();
#if HZ10_ENABLE_SHIM_THREAD_EXIT_STATS
  ...
  if (hz10_shim_orphan_registry_drain_probe) {
    (void)atexit(hz10_shim_dump_orphan_registry_drain_probe);
  }
