#!/usr/bin/env bash
# HZ11Sh6benchSpanPageFootprintWithBatch32-L1: page/span attribution.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "${ROOT}/.." && pwd)"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUTDIR="${OUTDIR:-${REPO_ROOT}/bench_results/hz11_sh6bench_span_page_batch32_${STAMP}}"
RUNS="${RUNS:-3}"
BUILD="${BUILD:-1}"
...
classes=list(csv.DictReader(open(sys.argv[3], newline="")))
sg=defaultdict(list)
cg=defaultdict(list)
kg=defaultdict(list)
...
with open(sys.argv[4],"w") as f:
    ...
    f.write("## Samples\n\n")
    f.write("| Allocator | status | median wall sec | wall/tcmalloc | median max RSS KiB | max RSS/tcmalloc | median current RSS KiB | xfer_hit | xfer_miss | xfer_insert | xfer_spill | central_hit | central_miss | central_insert | central_final_total | central_high_water_max | span_create |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for alloc in order:
        if alloc not in sg:
            continue
        ...
        f.write(f"| {alloc} | {status(items)} | {fmt(wall)} | {fmt(wall/base_wall if wall and base_wall else None)} | {fmt(rss)} | {fmt(rss/base_rss if rss and base_rss else None)} | {fmt(cur)} | {total(items,'xfer_hit')} | {total(items,'xfer_miss')} | {total(items,'xfer_insert')} | {total(items,'xfer_spill')} | {total(items,'central_hit')} | {total(items,'central_miss')} | {total(items,'central_insert')} | {total(cg.get(alloc,[]),'count')} | {maxv(cg.get(alloc,[]),'high_water')} | {total(items,'span_create')} |\n")
    f.write("\n## Source Class Totals\n\n")
    f.write("| Allocator | class | span_create | arena_carve | span_reuse | live_current_span | created_high_water | transfer_refill_hit | transfer_refill_miss | central_refill_hit | central_refill_miss | current_span_exhaust | current_span_replace |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for (alloc, cls), items in sorted(kg.items(), key=lambda kv: (kv[0][0], -total(kv[1],"span_create"))):
        if total(items, "span_create") == 0 and total(items, "arena_carve") == 0:
            continue
        f.write(f"| {alloc} | {cls} | {total(items,'span_create')} | {total(items,'arena_carve')} | {total(items,'span_reuse')} | {total(items,'live_current_span')} | {maxv(items,'created_high_water')} | {total(items,'transfer_refill_hit')} | {total(items,'transfer_refill_miss')} | {total(items,'central_refill_hit')} | {total(items,'central_refill_miss')} | {total(items,'current_span_exhaust')} | {total(items,'current_span_replace')} |\n")
    f.write("\n## Central Classes\n\n")
    ...
    f.write("\nRaw samples: `samples.csv`; raw central classes: `central_classes.csv`; raw source classes: `class_source.csv`\n")
PY

echo "[DONE] HZ11 sh6bench span/page logs saved to ${OUTDIR}"
