#!/usr/bin/env bash
# HZ11Sh6benchRequestSizeClassPackingObservation-L1: diagnostics only.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "${ROOT}/.." && pwd)"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUTDIR="${OUTDIR:-${REPO_ROOT}/bench_results/hz11_sh6bench_request_class_packing_${STAMP}}"
RUNS="${RUNS:-3}"
BUILD="${BUILD:-1}"
...
if [[ "${BUILD}" -ne 0 ]]; then
  make -C "${ROOT}" \
    ...
    preload-span-transfer-thread-exit-cap-batch32-live-diag >/dev/null
fi
...
run_sampled() {
  ...
  case "${diag}" in
    stats) cmd+=(HZ11_DUMP_STATS=1 HZ11_DUMP_CENTRAL_CLASSES=1) ;;
    live) cmd+=(HZ11_DUMP_STATS=1 HZ11_DUMP_CENTRAL_CLASSES=1 \
      HZ11_DUMP_SPAN_SOURCE=1 HZ11_DUMP_LIVE_FOOTPRINT=1) ;;
  esac
...
for run in $(seq 1 "${RUNS}"); do
  ...
  run_sampled hz11-thread-exit-cap-batch32 "${run}" \
...
order=["tcmalloc","hz11-thread-exit-cap-batch32","hz11-thread-exit-cap-batch32-live-diag"]
base_wall=med(sg.get("tcmalloc",[]),"wall_sec")
base_rss=med(sg.get("tcmalloc",[]),"max_rss_kb")
with open(sys.argv[5],"w") as f:
    f.write("# HZ11 Sh6bench Request Size Class Packing Observation L1\n\n")
    f.write("Diagnostics only. Batch32 behavior is unchanged; request counters are in the live-diag sibling.\n\n")
    f.write("## Samples\n\n")
    f.write("| Allocator | status | median wall sec | wall/tcmalloc | median max RSS KiB | max RSS/tcmalloc | median current RSS KiB | xfer_hit | xfer_miss | xfer_insert | xfer_spill | central_insert | span_create |\n")
    f.write("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    for alloc in order:
        if alloc not in sg:
            continue
    ...
    f.write("|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    alloc="hz11-thread-exit-cap-batch32-live-diag"
    classes=sorted({cls for a, cls in set(lv) | set(so) if a == alloc}, key=lambda x:int(x))
    total_req_bytes=0; total_slot_bytes=0; total_slot_waste=0
    total_capacity=0; total_span_waste=0
    for cls in classes:
        live_items=lv.get((alloc, cls), [])
        source_items=so.get((alloc, cls), [])
        slot=maxv(live_items,"slot")
        slot_high=maxv(live_items,"bytes_high")
        req_high=maxv(live_items,"req_bytes_high")
        slot_waste=max(0, slot_high - req_high)
        slot_ratio=(slot_waste / slot_high) if slot_high else None
        span_create=maxv(source_items,"span_create")
        capacity=span_create * 65536
        span_waste=max(0, capacity - slot_high)
        if slot_high == 0 and span_create == 0:
            continue
        total_req_bytes += req_high
        total_slot_bytes += slot_high
        total_slot_waste += slot_waste
        total_capacity += capacity
        total_span_waste += span_waste
        f.write(f"| {cls} | {slot} | {total(live_items,'alloc')} | {total(live_items,'free')} | {maxv(live_items,'live_high')} | {req_high} | {slot_high} | {slot_waste} | {fmt(slot_ratio)} | {span_create} | {capacity} | {span_waste} |\n")
    total_ratio=(total_slot_waste / total_slot_bytes) if total_slot_bytes else None
    f.write(f"| total | - | - | - | - | {total_req_bytes} | {total_slot_bytes} | {total_slot_waste} | {fmt(total_ratio)} | - | {total_capacity} | {total_span_waste} |\n")
    f.write("\n## Source And Central Class Totals\n\n")
    f.write("| class | arena carve | span reuse | live current span | created high water | transfer refill hit | transfer refill miss | central final count | central high water |\n")
    f.write("|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    classes=sorted({cls for a, cls in set(so) | set(cg) if a == alloc}, key=lambda x:int(x))
    for cls in classes:
        si=so.get((alloc, cls), []); ci=cg.get((alloc, cls), [])
        if total(si,"arena_carve") == 0 and maxv(ci,"high_water") == 0:
            continue
        f.write(f"| {cls} | {total(si,'arena_carve')} | {total(si,'span_reuse')} | {total(si,'live_current_span')} | {maxv(si,'created_high_water')} | {total(si,'transfer_refill_hit')} | {total(si,'transfer_refill_miss')} | {total(ci,'count')} | {maxv(ci,'high_water')} |\n")
    f.write("\nRaw samples: `samples.csv`; raw request/class packing: `live_footprint.csv`; raw source classes: `class_source.csv`; raw central classes: `central_classes.csv`\n")
PY

echo "[DONE] HZ11 sh6bench request/class-packing logs saved to ${OUTDIR}"
