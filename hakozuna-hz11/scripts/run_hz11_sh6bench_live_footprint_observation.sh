#!/usr/bin/env bash
# HZ11Sh6benchLiveFootprintObservation-L1: diagnostics only.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "${ROOT}/.." && pwd)"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUTDIR="${OUTDIR:-${REPO_ROOT}/bench_results/hz11_sh6bench_live_footprint_${STAMP}}"
RUNS="${RUNS:-3}"
BUILD="${BUILD:-1}"
...
printf 'allocator,run,class,slot,alloc,free,live,live_high,bytes_live,bytes_high\n' >"${live}"
...
with open(sys.argv[5],"w") as f:
    f.write("# HZ11 Sh6bench Live Footprint Observation L1\n\n")
    f.write("Diagnostics only. Batch32 behavior is unchanged; live counters are in the live-diag sibling.\n\n")
    f.write("## Samples\n\n")
    ...
    f.write("\n## Live Class Footprint\n\n")
    f.write("| class | slot | alloc total | free total | live | live high | live bytes high | span create high | span capacity bytes | estimated internal waste | waste ratio |\n")
    f.write("|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|\n")
    alloc="hz11-thread-exit-cap-batch32-live-diag"
    classes=sorted({cls for a, cls in set(lv) | set(so) if a == alloc}, key=lambda x:int(x))
    total_live_bytes=0; total_capacity=0; total_waste=0
    for cls in classes:
        live_items=lv.get((alloc, cls), [])
        source_items=so.get((alloc, cls), [])
        slot=maxv(live_items,"slot")
        bytes_high=maxv(live_items,"bytes_high")
        span_create=maxv(source_items,"span_create")
        capacity=span_create * 65536
        waste=max(0, capacity - bytes_high)
        ratio=(waste / capacity) if capacity else None
        if bytes_high == 0 and span_create == 0:
            continue
        total_live_bytes += bytes_high
        total_capacity += capacity
        total_waste += waste
        f.write(f"| {cls} | {slot} | {total(live_items,'alloc')} | {total(live_items,'free')} | {maxv(live_items,'live')} | {maxv(live_items,'live_high')} | {bytes_high} | {span_create} | {capacity} | {waste} | {fmt(ratio)} |\n")
    f.write(f"| total | - | - | - | - | - | {total_live_bytes} | - | {total_capacity} | {total_waste} | {fmt(total_waste/total_capacity if total_capacity else None)} |\n")
    ...
    f.write("\nRaw samples: `samples.csv`; raw live footprint: `live_footprint.csv`; raw source classes: `class_source.csv`; raw central classes: `central_classes.csv`\n")
PY

echo "[DONE] HZ11 sh6bench live-footprint logs saved to ${OUTDIR}"
