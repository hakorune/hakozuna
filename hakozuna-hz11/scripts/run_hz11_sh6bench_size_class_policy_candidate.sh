#!/usr/bin/env bash
# HZ11Sh6benchSizeClassPolicyCandidate-L1: opt-in fine size-class candidate.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "${ROOT}/.." && pwd)"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUTDIR="${OUTDIR:-${REPO_ROOT}/bench_results/hz11_sh6bench_size_class_policy_${STAMP}}"
RUNS="${RUNS:-3}"
BUILD="${BUILD:-1}"
...
with open(sys.argv[5],"w") as f:
    f.write("# HZ11 Sh6bench Size Class Policy Candidate L1\n\n")
    f.write("Fineclass is an opt-in size-class policy candidate. Live diagnostic rows include request-size side-table overhead and are for class-packing attribution only.\n\n")
    f.write("## Samples\n\n")
    ...
    for alloc in ("hz11-thread-exit-cap-batch32-live-diag", "hz11-thread-exit-cap-batch32-fineclass-live-diag"):
        ...
        for cls in classes:
            ...
            if total(si,"arena_carve") == 0 and maxv(ci,"high_water") == 0:
                continue
            f.write(f"| {alloc} | {cls} | {total(si,'arena_carve')} | {total(si,'span_reuse')} | {total(si,'live_current_span')} | {maxv(si,'created_high_water')} | {total(si,'transfer_refill_hit')} | {total(si,'transfer_refill_miss')} | {total(ci,'count')} | {maxv(ci,'high_water')} |\n")
...
echo "[DONE] HZ11 sh6bench size-class-policy logs saved to ${OUTDIR}"
