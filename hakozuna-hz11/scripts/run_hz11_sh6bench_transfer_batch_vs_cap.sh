#!/usr/bin/env bash
# HZ11Sh6benchTransferBatchVsCap-L1: split transfer batch vs cap effects.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
REPO_ROOT="$(cd "${ROOT}/.." && pwd)"
STAMP="${STAMP:-$(date -u +%Y%m%dT%H%M%SZ)}"
OUTDIR="${OUTDIR:-${REPO_ROOT}/bench_results/hz11_sh6bench_transfer_batch_vs_cap_${STAMP}}"
RUNS="${RUNS:-3}"
BUILD="${BUILD:-1}"
...
if [[ "${BUILD}" -ne 0 ]]; then
  make -C "${ROOT}" \
    ...
    preload-span-transfer-thread-exit-cap-xferwide >/dev/null
fi
...
for run in $(seq 1 "${RUNS}"); do
  ...
  run_sampled hz11-thread-exit-cap "${run}" "${ROOT}/libhz11_span_transfer_thread_exit_cap.so"
  run_sampled hz11-thread-exit-cap-batch32 "${run}" "${ROOT}/libhz11_span_transfer_thread_exit_cap_batch32.so"
  run_sampled hz11-thread-exit-cap-cap8192 "${run}" "${ROOT}/libhz11_span_transfer_thread_exit_cap_cap8192.so"
  run_sampled hz11-thread-exit-cap-xferwide "${run}" "${ROOT}/libhz11_span_transfer_thread_exit_cap_xferwide.so"
done
...
order=[
    ...
    "hz11-thread-exit-cap-cap8192",
    "hz11-thread-exit-cap-xferwide",
]
with open(sys.argv[3],"w") as f:
    f.write("# HZ11 Sh6bench Transfer Batch Vs Cap L1\n\n")
...
echo "[DONE] HZ11 sh6bench batch-vs-cap logs saved to ${OUTDIR}"
