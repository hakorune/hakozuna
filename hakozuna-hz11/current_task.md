...
```text
Active status:
  HZ11MacroSpeedLaneGateWithThreadExit-L1 is measured as macro NO-GO.
...
Macro promotion blockers:
  python_alloc and mstress still fail fast under the thread-exit candidate.
...
Goal:
  address python_alloc/mstress central capacity fail-fast without depending on
  current-span thread-exit pooling.

Boundary:
  central policy only; keep transfer/thread-exit as opt-in candidate lanes.

Required evidence:
  focused python_alloc/mstress pass evidence before another macro promotion gate.
