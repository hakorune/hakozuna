...
#define ALLOC_SIZE 4096  // Size class 0
...
static void* thread_func(void* arg) {
    ...
    // Phase 1: Allocate and store in shared array
    for (int i = 0; i < ALLOCS_PER_THREAD; i++) {
        void* ptr = malloc(ALLOC_SIZE);
        if (!ptr) {
            fprintf(stderr, "T%d: malloc failed at i=%d\n", tid, i);
            return (void*)1;
        }
        // Touch memory to ensure it's mapped
        memset(ptr, tid, ALLOC_SIZE);
...
int main(void) {
    ...
    printf("OK: MT remote free test passed (T=%d, allocs=%d each)\n",
