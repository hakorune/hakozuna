...
$SummaryPath = Join-Path $OutputDir ($Stamp + "_memcached_min_throughput.md")
$RawLogPath = Join-Path $RawOutputDir ($Stamp + "_memcached_min_throughput.log")
$StdoutPath = Join-Path $RawOutputDir ($Stamp + "_memcached_min_throughput_stdout.log")
$StderrPath = Join-Path $RawOutputDir ($Stamp + "_memcached_min_throughput_stderr.log")

if (-not ("HzMemcachedMiniBench" -as [type])) {
...
public sealed class HzBenchResult {
...
public static class HzMemcachedMiniBench {
    private static string ReadGetResponse(StreamReader reader) {
        ...
        if (header == "END") {
            throw new IOException("cache miss during get benchmark");
        }
    ...
    private static void RunSetWorker(string host, int port, int clientId, int requestsPerClient, int keySpace, string payload) {
    ...
    private static void RunGetWorker(string host, int port, int clientId, int requestsPerClient, int keySpace, string expectedPayload) {
        using (TcpClient client = new TcpClient()) {
            client.NoDelay = true;
            client.ReceiveTimeout = 5000;
            client.SendTimeout = 5000;
            ...
            using (StreamReader reader = new StreamReader(stream, Encoding.ASCII, false, 4096, true)) {
                ...
                for (int i = 0; i < requestsPerClient; i++) {
                    ...
                    if (value != expectedPayload) {
                        throw new IOException("unexpected get payload");
                    }
                }
                writer.Write("quit\r\n");
                writer.Flush();
    ...
    public static HzBenchResult RunSetPhase(string host, int port, int clients, int requestsPerClient, string payload) {
        int keySpace = clients * requestsPerClient;
        Stopwatch sw = Stopwatch.StartNew();
        Task[] tasks = new Task[clients];
        for (int i = 0; i < clients; i++) {
            int clientId = i;
            tasks[i] = Task.Run(() => RunSetWorker(host, port, clientId, requestsPerClient, keySpace, payload));
        }
        Task.WaitAll(tasks);
        sw.Stop();
        return new HzBenchResult {
            Phase = "set",
            TotalOps = clients * requestsPerClient,
            ElapsedSeconds = sw.Elapsed.TotalSeconds,
            OpsPerSecond = (clients * requestsPerClient) / sw.Elapsed.TotalSeconds
        };
    }

    public static HzBenchResult RunGetPhase(string host, int port, int clients, int requestsPerClient, string payload) {
        int keySpace = clients * requestsPerClient;
        Stopwatch sw = Stopwatch.StartNew();
        Task[] tasks = new Task[clients];
        for (int i = 0; i < clients; i++) {
            int clientId = i;
            tasks[i] = Task.Run(() => RunGetWorker(host, port, clientId, requestsPerClient, keySpace, payload));
        }
        Task.WaitAll(tasks);
        sw.Stop();
        return new HzBenchResult {
...
try {
    ...
    $getResult = [HzMemcachedMiniBench]::RunGetPhase($Listen, $Port, $Clients, $RequestsPerClient, $Payload)
    ...
    $raw.Add(("set_ops_per_sec: {0:N2}" -f $setResult.OpsPerSecond))
    $raw.Add(("get_ops_per_sec: {0:N2}" -f $getResult.OpsPerSecond))
    $raw.Add("")

    $summary.Add("# Windows Memcached Min Throughput")
    $summary.Add("")
    ...
    $summary.Add("| --- | ---: | ---: | ---: |")
    $summary.Add(('| set | {0} | {1:N3} | {2:N2} |' -f $setResult.TotalOps, $setResult.ElapsedSeconds, $setResult.OpsPerSecond))
    $summary.Add(('| get | {0} | {1:N3} | {2:N2} |' -f $getResult.TotalOps, $getResult.ElapsedSeconds, $getResult.OpsPerSecond))
    $summary.Add("")
    $summary.Add("Config:")
