...
$Executables = @(
    ...
    @{ Name = "hz11-span-cache512"; Path = (Join-Path $SuiteDir "bench_random_mixed_hz11_span_cache512.exe") },
    @{ Name = "hz11-span-cache512-classbatch"; Path = (Join-Path $SuiteDir "bench_random_mixed_hz11_span_cache512_classbatch.exe") },
...
$Summary.Add('- `hz11-span` is the Windows L1 control row: VirtualAlloc-backed span/classify, no transfer/fine128 parity claim, and no DLL replacement claim.')
$Summary.Add('- `hz11-span-cache256` is the selected Windows HZ11 bring-up row: `hz11-span` plus `HZ11_CACHE_CAP=256`, selected after matrix attribution showed returned-object overflow pressure.')
