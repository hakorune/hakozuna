...
if (-not $RawOutputDir) {
    $RawOutputDir = Join-Path $RepoRoot "private\raw-results\windows\redis\real_profile_matrix"
}
...
$AllProfiles = @(
    @{ Name = "balanced"; Requests = 100000; Clients = 50; Pipeline = 16; Tests = @("SET", "GET", "LPUSH", "LPOP") },
    @{ Name = "lowpipe"; Requests = 50000; Clients = 40; Pipeline = 1; Tests = @("SET", "GET", "LPUSH", "LPOP") },
    @{ Name = "highpipe"; Requests = 200000; Clients = 80; Pipeline = 32; Tests = @("SET", "GET", "LPUSH", "LPOP") },
    @{ Name = "kv_only"; Requests = 150000; Clients = 80; Pipeline = 16; Tests = @("SET", "GET") },
    @{ Name = "list_only"; Requests = 150000; Clients = 60; Pipeline = 16; Tests = @("LPUSH", "LPOP") }
)

if ([string]::IsNullOrWhiteSpace($Profiles)) {
    $SelectedProfiles = $AllProfiles
} else {
    $requestedProfiles = @($Profiles.Split(",") | ForEach-Object { $_.Trim().ToLowerInvariant() } | Where-Object { $_ -ne "" })
    $SelectedProfiles = @($AllProfiles | Where-Object { $requestedProfiles -contains $_.Name.ToLowerInvariant() })
    if ($SelectedProfiles.Count -eq 0) {
        throw "No matching profiles selected: $Profiles"
    }
}
...
foreach ($profile in $SelectedProfiles) {
    ...
    foreach ($allocator in $AllocatorList) {
        $Matrix[$profile.Name][$allocator] = @{}
        foreach ($testName in $profile.Tests) {
            $Matrix[$profile.Name][$allocator][$testName] = New-Object System.Collections.Generic.List[double]
        }
    }
}

$Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$SummaryPath = Join-Path $OutputDir ($Stamp + "_redis_real_windows_profile_matrix.md")
$RawLogPath = Join-Path $RawOutputDir ($Stamp + "_redis_real_windows_profile_matrix.log")
$WorkRoot = Join-Path $RepoRoot "out_win_redis_real"
$Summary = New-Object System.Collections.Generic.List[string]
$Raw = New-Object System.Collections.Generic.List[string]
$Details = New-Object System.Collections.Generic.List[string]

$Summary.Add("# Windows Real Redis Profile Matrix")
$Summary.Add("")
...
$Summary.Add("- [win/run_win_redis_real_matrix.ps1](/C:/git/hakozuna-win/win/run_win_redis_real_matrix.ps1)")
$Summary.Add("- [win/run_win_redis_real_profile_matrix.ps1](/C:/git/hakozuna-win/win/run_win_redis_real_profile_matrix.ps1)")
...
$Summary.Add(('Runs per profile/allocator: `{0}`' -f $Runs))
$Summary.Add("- statistic: median requests/sec")
$Summary.Add("- raw logs: private only")
$Summary.Add("")
$Summary.Add("| profile | allocator | test | median requests/sec | clients | pipeline | requests | runs |")
$Summary.Add("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |")
...
for ($allocatorIndex = 0; $allocatorIndex -lt $AllocatorList.Count; $allocatorIndex++) {
    ...
    for ($profileIndex = 0; $profileIndex -lt $SelectedProfiles.Count; $profileIndex++) {
        $profile = $SelectedProfiles[$profileIndex]
        $port = $PortBase + ($allocatorIndex * 100) + $profileIndex
        ...
        for ($run = 1; $run -le $Runs; $run++) {
            ...
            try {
                ...
                $args = @(
                    ...
                    "-h", "127.0.0.1",
                    "-p", [string]$port,
                    "-t", (($profile.Tests | ForEach-Object { $_.ToLowerInvariant() }) -join ","),
                    "-n", [string]$profile.Requests,
                    "-P", [string]$profile.Pipeline,
                    "-c", [string]$profile.Clients
                )
                $output = & $RedisBench @args 2>&1
                $rc = $LASTEXITCODE

                $Raw.Add(("=== allocator {0} profile {1} run {2}/{3} ===" -f $allocator, $profile.Name, $run, $Runs))
                ...
                foreach ($line in $output) {
                    ...
                    if ($text -match "^(SET|GET|LPUSH|LPOP|INCR):\s+([0-9.]+)\s+requests per second$") {
                        ...
                        if ($Matrix[$profile.Name][$allocator].ContainsKey($name)) {
                            $Matrix[$profile.Name][$allocator][$name].Add($value)
                        }
                    }
                }
            } finally {
...
foreach ($profile in $SelectedProfiles) {
    foreach ($allocator in $AllocatorList) {
        foreach ($testName in $profile.Tests) {
            $runsList = $Matrix[$profile.Name][$allocator][$testName]
            $median = Get-Median -Values $runsList.ToArray()
            $Summary.Add(('| {0} | {1} | {2} | {3:N2} | {4} | {5} | {6} | {7} |' -f `
                $profile.Name, $allocator.ToLowerInvariant(), $testName.ToLowerInvariant(), $median,
                $profile.Clients, $profile.Pipeline, $profile.Requests, $Runs))
            $runText = ($runsList | ForEach-Object { ('{0:N2}' -f $_) }) -join ", "
            $Details.Add(('- `{0} / {1} / {2}`: [{3}]' -f `
                $profile.Name, $allocator.ToLowerInvariant(), $testName.ToLowerInvariant(), $runText))
        }
    }
}
