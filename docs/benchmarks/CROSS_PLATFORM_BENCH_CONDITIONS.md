...
- For HZ6 remote-route repair, keep the Ubuntu and Windows work in the same
  `RemoteFreeRouteResolve-L1` box. `main_r50`, `main_r90`, and `cross128_r90`
  must use the shared core resolver, must not enter local or visible page-table
  route scans for proven foreign frees, and must split `PROVEN_EXTERNAL` from
  `UNRESOLVED_INTEGRITY`.  The promotion gate is RUNS=10, `T=16`, `ws=100`,
  `ring_slots=65536`, with no abort, timeout, or unproven real-free for
  HZ6-owned pointers on Ubuntu x86_64 and Windows x64.  Use
