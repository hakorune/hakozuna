...
- hakozuna-hz8/: HZ8 (KeepRefill + Mag16 + GeneralMediumPage + EntryBoundary、低い post-workload RSS、fail-closed ownership を持つ推奨 balanced allocator line)
...
- hakozuna-hz11/: HZ11 (design-only speed-first line。tcmalloc型 front/cache/span 設計)
- hakozuna-hz12/: HZ12 (advisory ownership と bounded span reclamation の研究 line)
...
| HZ8 | 推奨 balanced line | fail-closed ownership + owner-stable remote free + KeepRefill + Mag16 + GeneralMediumPage | 現在の公開 allocator line。旧挙動は hz8-v2-rollback |
...
| HZ11 | speed-first tcmalloc competitor design | front-end cache + transfer cache + central spans + checked-mode diagnostics | design-only speed line |
| HZ12 | bounded whole-span reclamation | advisory ownership + complete-span authority + bounded depot + zero-limbo rollback | cold-boundary reclaim 研究 line |
...
- HZ11 は design-only の speed-first line。HZ8/HZ10 の correctness/RSS 境界とは別に、tcmalloc型の速度勝負を設計する。
- HZ12 は HZ11 の ownerless speed baseline に cold-boundary ownership と
  bounded whole-span reclaim を加える研究 line。HZ8 の公開推奨を置き換えない。
...
HZ11 は別ラインで、tcmalloc型の front-end cache / transfer cache / span
allocator をHakozuna内で設計する speed-first 研究です。
