...
if (-not $SkipHz3Build) {
    if ($RebuildHz3 -or -not (Test-Path $Hz3Lib)) {
        ...
        $BuildArgs = @()
        if ($TlsDeclspec) { $BuildArgs += "-TlsDeclspec" }
        if ($TlsEmulated) { $BuildArgs += "-TlsEmulated" }
        if ($TlsInitLog) { $BuildArgs += "-TlsInitLog" }
        if ($TlsInitFailfast) { $BuildArgs += "-TlsInitFailfast" }
        if ($SmallSlowStats) { $BuildArgs += "-SmallSlowStats" }
        if ($ArenaFailShot) { $BuildArgs += "-ArenaFailShot" }
        if ($OomShot) { $BuildArgs += "-OomShot" }
        & $BuildHz3 @BuildArgs | Out-Null
        if ($LASTEXITCODE -ne 0) {
