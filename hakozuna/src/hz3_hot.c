...
#else
#if HZ3_MALLOC_FIRST_LOG
    static int g_hz3_malloc_first = 0;
    if (__builtin_expect(g_hz3_malloc_first == 0, 0)) {
        g_hz3_malloc_first = 1;
        FILE* log = fopen("hz3_malloc_first.log", "w");
        if (log) {
            fprintf(log, "hz3_malloc first call: size=%zu\n", size);
#if HZ3_SMALL_V2_ENABLE && HZ3_SEG_SELF_DESC_ENABLE
            fprintf(log, "HZ3_SMALL_V2_ENABLE=1 HZ3_SEG_SELF_DESC_ENABLE=1\n");
#else
            fprintf(log, "HZ3_SMALL_V2_ENABLE=0 or HZ3_SEG_SELF_DESC_ENABLE=0\n");
#endif
#if HZ3_S85_SMALL_V2_SLOW_STATS
            fprintf(log, "HZ3_S85_SMALL_V2_SLOW_STATS=1\n");
#else
            fprintf(log, "HZ3_S85_SMALL_V2_SLOW_STATS=0 or undefined\n");
#endif
#if HZ3_ARENA_ALLOC_FAIL_SHOT
            fprintf(log, "HZ3_ARENA_ALLOC_FAIL_SHOT=1\n");
#else
            fprintf(log, "HZ3_ARENA_ALLOC_FAIL_SHOT=0 or undefined\n");
#endif
            fclose(log);
        }
    }
...
void hz3_free(void* ptr) {
    ...
    // NOTE: Must match hz3_free_try_s113_segmath() logic exactly!
    {
        ...
        // Step 1: Load arena_base (match S113 fast path)
        ...
        // Step 3: Compute page_idx, skip page 0 (match S113)
        ...
        // Step 4: Read page_bin_plus1 (match S113: no magic check!)
