...
static void hz3_s85_dump(void) {
    ...
    FILE* log = fopen("hz3_s85_stats.log", "w");
    if (log) {
        fprintf(log,
...
void hz3_s85_small_v2_slow_record(int sc,
    ...
    static _Atomic uint32_t g_first_call = 0;
    if (atomic_fetch_add_explicit(&g_first_call, 1, memory_order_relaxed) == 0) {
        FILE* log = fopen("hz3_s85_first_call.log", "w");
        if (log) {
            fprintf(log, "S85 stats recording started: sc=%d\n", sc);
            fclose(log);
        }
    }
