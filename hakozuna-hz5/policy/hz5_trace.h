#ifndef HZ5_TRACE_H
#define HZ5_TRACE_H

#include <stdint.h>

#ifndef BENCHLAB_HZ5_TRACE_LANE
#define BENCHLAB_HZ5_TRACE_LANE 0
#endif

#ifndef BENCHLAB_HZ5_SPEED_LANE
#define BENCHLAB_HZ5_SPEED_LANE 0
#endif

#if BENCHLAB_HZ5_TRACE_LANE && BENCHLAB_HZ5_SPEED_LANE
#error "BENCHLAB_HZ5_TRACE_LANE must not be enabled in SPEED_LANE builds"
#endif

typedef enum Hz5TraceCounter {
  HZ5_TRACE_ALLOC_P25_BRIDGE = 0,
  HZ5_TRACE_ALLOC_P43_SOURCE_TLS,
  HZ5_TRACE_ALLOC_P43_SOURCE_COMMITTED,
  HZ5_TRACE_ALLOC_P43_SOURCE_RELEASE_BUFFER,
  HZ5_TRACE_ALLOC_P43_SOURCE_COLD,
  HZ5_TRACE_ALLOC_P43_SOURCE_FREE_SLOT,
  HZ5_TRACE_ALLOC_P43_SOURCE_NEW_SEGMENT,
  HZ5_TRACE_ALLOC_P43_TOKEN,
  HZ5_TRACE_FREE_P25_BRIDGE,
  HZ5_TRACE_FREE_P43_LOOKUP_PREPARED,
  HZ5_TRACE_FREE_P43_TOKEN_DIRECT,
  HZ5_TRACE_FREE_P43_TOKEN_BRIDGE,
  HZ5_TRACE_FREE_TRUSTWRAP,
  HZ5_TRACE_FREE_P25_BRIDGE_ATTR,
  HZ5_TRACE_FREE_RAWLOOKUP,
  HZ5_TRACE_FREE_FALLBACK_OR_INVALID,
  HZ5_TRACE_WRAPPER_DECODE_OK,
  HZ5_TRACE_WRAPPER_DECODE_MISS,
  HZ5_TRACE_WRAPPER_TOKEN_VALID,
  HZ5_TRACE_WRAPPER_TOKEN_INVALID,
  HZ5_TRACE_BRIDGE_ATTR_VALID,
  HZ5_TRACE_BRIDGE_ATTR_INVALID,
  HZ5_TRACE_BRIDGE_ATTR_CAS_FAIL,
  HZ5_TRACE_ALLOC_LOCAL2P_TLS_HIT,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_HIT,
  HZ5_TRACE_ALLOC_LOCAL2P_GLOBAL_HIT,
  HZ5_TRACE_ALLOC_LOCAL2P_OS,
  HZ5_TRACE_ALLOC_LOCAL2P_ESCAPE,
  HZ5_TRACE_ALLOC_LOCAL2P_EXACT_HIT,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_FAST_ACTIVATE,
  HZ5_TRACE_ALLOC_LOCAL2P_ESCAPE_SIZE,
  HZ5_TRACE_ALLOC_LOCAL2P_ESCAPE_ALIGN,
  HZ5_TRACE_ALLOC_LOCAL2P_ESCAPE_GUARD,
  HZ5_TRACE_FREE_LOCAL2P_TLS,
  HZ5_TRACE_FREE_LOCAL2P_INBOX,
  HZ5_TRACE_FREE_LOCAL2P_GLOBAL,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE,
  HZ5_TRACE_FREE_LOCAL2P_INVALID_COOKIE,
  HZ5_TRACE_FREE_LOCAL2P_INVALID_STATE,
  HZ5_TRACE_FREE_LOCAL2P_OVERFLOW,
  HZ5_TRACE_ALLOC_LOCAL2P_TLS_FAST_RETURN,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_CACHE_HIT,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_EXCHANGE,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_EXCHANGE_NODES,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_CACHE_DEPTH_MAX,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_CACHE_HIT_STREAK_MAX,
  HZ5_TRACE_ALLOC_LOCAL2P_INBOX_POST_FLUSH_REUSE_MAX,
  HZ5_TRACE_ALLOC_LOCAL2P_HEADER_INIT,
  HZ5_TRACE_FREE_LOCAL2P_SAME_OWNER_FAST,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_ENQUEUE,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_OWNER,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_CAP,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_SWITCH,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_NODES,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_OWNER_NODES,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_CAP_NODES,
  HZ5_TRACE_FREE_LOCAL2P_REMOTE_BATCH_FLUSH_SWITCH_NODES,
  HZ5_TRACE_LOCAL2P_REMOTE_OWNER_SWITCH,
  HZ5_TRACE_LOCAL2P_REMOTE_OWNER_GATE_SKIP,
  HZ5_TRACE_LOCAL2P_REMOTE_DIRECT_INDEX_HIT,
  HZ5_TRACE_LOCAL2P_REMOTE_DIRECT_INDEX_COLLISION,
  HZ5_TRACE_LOCAL2P_REMOTE_ASSOC_LRU_VICTIM,
  HZ5_TRACE_LOCAL2P_REMOTE_ACTIVE_SLOTS_MAX,
  HZ5_TRACE_LOCAL2P_REMOTE_BATCH_DEPTH_MAX,
  HZ5_TRACE_LOCAL2P_GLOBAL_POP_LOCK,
  HZ5_TRACE_LOCAL2P_GLOBAL_PUSH_LOCK,
  HZ5_TRACE_LOCAL2P_GLOBAL_PUSH_ACCEPT,
  HZ5_TRACE_LOCAL2P_GLOBAL_PUSH_REJECT,
  HZ5_TRACE_LOCAL2P_GLOBAL_COUNT_MAX,
  HZ5_TRACE_LOCAL2P_TLS_COUNT_MAX,
  HZ5_TRACE_LOCAL2P_REMOTE_SLOT_MATCH,
  HZ5_TRACE_LOCAL2P_REMOTE_SLOT_EMPTY,
  HZ5_TRACE_LOCAL2P_REMOTE_SLOT_VICTIM,
  HZ5_TRACE_LOCAL2P_ROUTE_COOKIE_OK,
  HZ5_TRACE_LOCAL2P_ROUTE_COOKIE_MISS,
  HZ5_TRACE_LOCAL2P_SIDECAR_INSERT,
  HZ5_TRACE_LOCAL2P_SIDECAR_INSERT_FAIL,
  HZ5_TRACE_LOCAL2P_SIDECAR_REMOVE,
  HZ5_TRACE_LOCAL2P_SIDECAR_HIT_VALID,
  HZ5_TRACE_LOCAL2P_SIDECAR_HIT_INVALID,
  HZ5_TRACE_LOCAL2P_SIDECAR_MISS,
  HZ5_TRACE_LOCAL2P_SIDECAR_PROBE_STEPS,
  HZ5_TRACE_LOCAL2P_SIDECAR_PROBE_STEPS_MAX,
  HZ5_TRACE_LOCAL2P_SIDECAR_COLLISION,
  HZ5_TRACE_LOCAL2P_SIDECAR_INSERT_COLLISION,
  HZ5_TRACE_LOCAL2P_SIDECAR_PROBE_COLLISION,
  HZ5_TRACE_LOCAL2P_SIDECAR_PROBE_EMPTY,
  HZ5_TRACE_LOCAL2P_SIDECAR_REMOVE_MISS,
  HZ5_TRACE_LOCAL2P_SIDECAR_REMOVE_SCAN_STEPS,
  HZ5_TRACE_LOCAL2P_SIDECAR_ASSOC_VICTIM,
  HZ5_TRACE_LOCAL2P_SIDECAR_FIRST_OK,
  HZ5_TRACE_LOCAL2P_SIDECAR_FIRST_INVALID,
  HZ5_TRACE_LOCAL2P_SIDECAR_FIRST_MISS,
  HZ5_TRACE_P20_CRT_BYPASS_ALLOC,
  HZ5_TRACE_P20_CRT_BYPASS_FREE,
  HZ5_TRACE_P20_CRT_BYPASS_ALLOC_BYTES,
  HZ5_TRACE_P20_CRT_BYPASS_FREE_BYTES,
  HZ5_TRACE_LOCAL2P_OS_ALLOC_BYTES,
  HZ5_TRACE_LOCAL2P_OS_FREE_BYTES,
  HZ5_TRACE_COUNT
} Hz5TraceCounter;

#if BENCHLAB_HZ5_TRACE_LANE
void hz5_trace_register_once(void);
void hz5_trace_inc(Hz5TraceCounter counter);
void hz5_trace_add(Hz5TraceCounter counter, uint64_t value);
void hz5_trace_max(Hz5TraceCounter counter, uint64_t value);
void hz5_trace_print_once(void);
#else
static inline void hz5_trace_register_once(void) {}
static inline void hz5_trace_inc(Hz5TraceCounter counter) {
  (void)counter;
}
static inline void hz5_trace_add(Hz5TraceCounter counter, uint64_t value) {
  (void)counter;
  (void)value;
}
static inline void hz5_trace_max(Hz5TraceCounter counter, uint64_t value) {
  (void)counter;
  (void)value;
}
static inline void hz5_trace_print_once(void) {}
#endif

#endif
