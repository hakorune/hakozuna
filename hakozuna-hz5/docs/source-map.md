...
alignment/layout-fit checks, freelist push/pop mechanics, generation wrap
handling, owner-token initialization, header activation, and ACTIVE-to-FREED
transition while leaving Linux and Windows allocation sources untouched. It
also centralizes the Local2P metadata-present predicate used by lane probes.
...
state transitions (push/reset), while Linux and Windows still keep their own
