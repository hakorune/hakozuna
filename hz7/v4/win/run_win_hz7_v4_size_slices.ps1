...
$RepoRoot = Split-Path -Parent $Hz7V4Root
...
$BenchDirName = if ($BenchSuffix -ne "") {
    "out_win_random_mixed_hz7v4$BenchSuffix"
} else {
    "out_win_random_mixed"
}
$BenchDir = Join-Path $RepoRoot $BenchDirName
$BenchExe = Join-Path $BenchDir "bench_random_mixed_hz7_v4.exe"

if (-not $OutputDir) {
    $OutputDir = Join-Path $RepoRoot "out_win_hz7_v4_size_slices"
}

New-Item -ItemType Directory -Force $OutputDir | Out-Null

if ($BenchSuffix -ne "" -or -not (Test-Path $BenchExe)) {
    & $BuildScript -OnlyHz7V4 -OutDirName $BenchDirName -Hz7V4DirectRetainCap $DirectRetainCap -Hz7V4EmptySpanCap $EmptySpanCap
...
$SummaryPath = Join-Path $OutputDir ($Stamp + "_hz7_v4_size_slices_windows.md")
$RawLogPath = Join-Path $OutputDir ($Stamp + "_hz7_v4_size_slices_windows.log")
$RawLines = New-Object System.Collections.Generic.List[string]
$Summary = New-Object System.Collections.Generic.List[string]

$Summary.Add("# HZ7 v4 Windows Size Slices")
$Summary.Add("")
$Summary.Add("Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz')")
$Summary.Add("")
$Summary.Add('- benchmark: `bench_random_mixed_compare`')
$Summary.Add('- allocator: `hz7-v4`')
