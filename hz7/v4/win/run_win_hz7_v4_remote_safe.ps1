...
if (-not $OutputDir) {
    $OutputDir = Join-Path $RepoRoot "out_win_hz7_v4_hotpath"
}

New-Item -ItemType Directory -Force $OutputDir | Out-Null

$Compiler = Get-Command $CompilerPath -ErrorAction Stop
$BenchSource = Join-Path $PSScriptRoot "bench_hz7_v4_remote_safe.c"
$Hz7Source = Join-Path $Hz7Root "hz7.c"
$OutputPath = Join-Path $OutputDir "bench_hz7_v4_remote_safe.exe"
...
Write-Host "[hz7-hotpath] building bench_hz7_v4_remote_safe.exe"
...
$SummaryPath = Join-Path $OutputDir ($Stamp + "_hz7_v4_hotpath_windows.md")
$RawLogPath = Join-Path $OutputDir ($Stamp + "_hz7_v4_hotpath_windows.log")
...
$Summary.Add("# HZ7 v4 Windows Hot Path Microbench")
$Summary.Add("")
$Summary.Add("Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz')")
$Summary.Add("")
$Summary.Add('- benchmark: `bench_hz7_v4_remote_safe`')
