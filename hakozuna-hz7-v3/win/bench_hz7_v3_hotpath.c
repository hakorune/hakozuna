...
static void h7_run_route_invariant(const char* label,
  ...
  for (i = 0; i < iters; ++i) {
    ...
    live_ok += h7_route(ptr) == H7_ROUTE_VALID;
    h7_free(ptr);
    dead_ok += h7_route(ptr) == H7_ROUTE_INVALID;
    ++roundtrips;
  }
  elapsed = h7_seconds() - start;
  printf("hz7_hotpath: op=route_invariant label=%s size=%zu iters=%u ok=%u "
         "live_ok=%u dead_ok=%u time=%.6f ops/s=%.2f peak_kb=%zu\n",
         label,
         size,
         iters,
         roundtrips,
         ...
         dead_ok,
         elapsed,
         elapsed > 0.0 ? (double)roundtrips / elapsed : 0.0,
         h7_peak_working_set_kb());
}
...
int main(int argc, char** argv) {
  ...
  h7_run_malloc_free("span4k", 4096u, iters);
  h7_run_malloc_free("span8k", 8192u, iters);
  h7_run_malloc_free("span16k", 16384u, iters);
  h7_run_malloc_free("direct32k", 32768u, iters);

  h7_run_route_invariant("span4k", 4096u, iters);
  h7_run_route_invariant("span8k", 8192u, iters);
