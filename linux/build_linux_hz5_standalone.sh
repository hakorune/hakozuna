...
LINUX_LARGEFRONT_DRAIN_BULK_LOCAL=0
LINUX_LARGEFRONT_DRAIN_TRUST_REMOTE_STATE=0
...
LINUX_LARGEFRONT_TRANSFER128=0
LINUX_LARGEFRONT_TRANSFER128_TLS_FIRST=0
LINUX_LARGEFRONT_TRANSFER128_OWNER_SHARD=0
LINUX_LARGEFRONT_TRANSFER128_CONSUMER_SHARD=0
...
hz5_add_bool_define_if() {
  local var_name="$1"
  local define_name="$2"
  if [[ "${!var_name}" -eq 1 ]]; then
    COMMON_FLAGS+=("-D${define_name}=1")
  fi
}

hz5_add_bool_define_group() {
  local var_prefix="$1"
  local define_prefix="$2"
  shift 2
  local suffix
  for suffix in "$@"; do
    hz5_add_bool_define_if "${var_prefix}_${suffix}" \
      "${define_prefix}_${suffix}"
  done
}
...
hz5_add_u_define() {
  local define_name="$1"
  local value="$2"
  COMMON_FLAGS+=("-D${define_name}=${value}u")
}
...
if [[ "$LINUX_LOCAL2P" -eq 1 ]]; then
  COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LOCAL2P=1)
  hz5_add_u_define BENCHLAB_HZ5_LINUX_LOCAL2P_TLS_CAP \
    "$LINUX_LOCAL2P_TLS_CAP"
  hz5_add_u_define BENCHLAB_HZ5_LINUX_LOCAL2P_GLOBAL_CAP \
    "$LINUX_LOCAL2P_GLOBAL_CAP"
  hz5_add_bool_define_group LINUX_LOCAL2P BENCHLAB_HZ5_LINUX_LOCAL2P \
    TLS_PACKED DIRECT_ROUTE DIRECT_INIT NO_COOKIE NO_CAS OWNER_INBOX \
    REMOTE_BATCH OBJECT_NODE SAME_OWNER_FAST_STATE ROUTE_COOKIE \
    REUSE_STATE_ONLY SLIM_CHECK FAST_COOKIE FREE_FIRST TLS_FAST_RETURN \
    SINGLE_SLOT_TLS LOCAL_OVERFLOW_GLOBAL RETAIN_ARRAY
  ...
  if [[ "$LINUX_LOCAL2P_REMOTE_BATCH" -eq 1 ]]; then
    ...
    hz5_add_u_define BENCHLAB_HZ5_LINUX_LOCAL2P_REMOTE_BATCH_CAP \
      "$LINUX_LOCAL2P_REMOTE_BATCH_CAP"
  fi
fi
...
if [[ "$ENABLE_LINUX_P43" -eq 1 ]]; then
  ...
  hz5_add_bool_define_group LINUX_P43 BENCHLAB_HZ5_P43 \
    UNSAFE_NO_LOOKUP TRUST_FAST_LOOKUP TRUST_WRAPPER_SOURCE \
    DECODED_RAW_LOOKUP RAW_FAST_LOOKUP_ONLY RAW_ALLOCATED_LOOKUP_ONLY \
    WRAPPER_TOKEN WRAPPER_TOKEN_BRIDGE
fi

if [[ "$LINUX_SMALLFRONT_S1" -eq 1 ]]; then
  COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_SMALLFRONT_S1=1)
  hz5_add_u_define HZ5_SMALLFRONT_REMOTE_BATCH_CAP \
    "$LINUX_SMALLFRONT_REMOTE_BATCH_CAP"
  hz5_add_bool_define_group LINUX_SMALLFRONT BENCHLAB_HZ5_LINUX_SMALLFRONT \
    DRAIN_EMPTY_GATED REMOTE_OUTBOX
fi
if [[ "$LINUX_MIDPAGEFRONT_M2" -eq 1 ]]; then
  COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_MIDPAGEFRONT_M2=1)
  hz5_add_u_define HZ5_MIDPAGEFRONT_REMOTE_BATCH_CAP \
    "$LINUX_MIDPAGEFRONT_REMOTE_BATCH_CAP"
  hz5_add_bool_define_group LINUX_MIDPAGEFRONT \
    BENCHLAB_HZ5_LINUX_MIDPAGEFRONT REGION_ARRAY REMOTE_SHADOW \
    LOCAL_FAST_STATE TLS_REGION_CACHE TLS_HOT_SLOT LOCAL_ACTIVE_TRUST \
    SLOT_SWITCH NODELESS_RUN NODELESS_PTRCACHE NODELESS_STATS \
    UNSAFE_LOCAL_NOCHECK M4_MAGAZINE M4_REMOTE_PACKET M4_CROSS_DRAIN \
    M4_UNSAFE_ALLOC_ELIDE M4_UNSAFE_PTR_MAG M4_UNSAFE_FREE_ELIDE \
    M5_HIT_ONLY M6_DEFERRED_FREE M6_REMOTE_DEFERRED_FREE M7_REMOTE_TICKET \
    M8_OWNER_XFER M9_BUDGET_DRAIN M10_REMOTE_SLOT_RING \
    M11_REMOTE_DIRECT_CACHE PAGERUN PAGERUN_64K M4_FLAT_MAG_CAP \
    M4_OVERFLOW_ARRAY WIDE_32K_CLASS M4_STATS
  if [[ "$LINUX_MIDPAGEFRONT_M4_REMOTE_DRAIN_ON_HIT" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_MIDPAGEFRONT_M4_REMOTE_DRAIN_ON_HIT=1)
    hz5_add_u_define HZ5_MIDPAGEFRONT_REMOTE_DRAIN_HIT_INTERVAL \
      "$LINUX_MIDPAGEFRONT_M4_REMOTE_DRAIN_HIT_INTERVAL"
  fi
  ...
  if [[ "$LINUX_MIDPAGEFRONT_M6_DEFERRED_FREE" -eq 1 || \
    ...
    hz5_add_u_define HZ5_MIDPAGEFRONT_M6_RAW_CAP \
      "$LINUX_MIDPAGEFRONT_M6_RAW_CAP"
  fi
  if [[ "$LINUX_MIDPAGEFRONT_M7_REMOTE_TICKET" -eq 1 ]]; then
    hz5_add_u_define HZ5_MIDPAGEFRONT_M7_REMOTE_PAGE_CAP \
      "$LINUX_MIDPAGEFRONT_M7_REMOTE_PAGE_CAP"
  fi
  if [[ "$LINUX_MIDPAGEFRONT_M8_OWNER_XFER" -eq 1 ]]; then
    hz5_add_u_define HZ5_MIDPAGEFRONT_M8_XFER_PAGE_CAP \
      "$LINUX_MIDPAGEFRONT_M8_XFER_PAGE_CAP"
  fi
  ...
  if [[ "$LINUX_MIDPAGEFRONT_EMPTY_SLAB_RELEASE" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_MIDPAGEFRONT_EMPTY_SLAB_RELEASE=1)
    hz5_add_u_define HZ5_MIDPAGEFRONT_EMPTY_RETAIN_CAP \
      "$LINUX_MIDPAGEFRONT_EMPTY_RETAIN_CAP"
...
if [[ "$LINUX_MIDFRONT_M1" -eq 1 ]]; then
  ...
  hz5_add_u_define HZ5_MIDFRONT_MAX_BYTES "$LINUX_MIDFRONT_MAX_BYTES"
  hz5_add_u_define HZ5_MIDFRONT_REMOTE_BATCH_CAP \
    "$LINUX_MIDFRONT_REMOTE_BATCH_CAP"
  hz5_add_bool_define_group LINUX_MIDFRONT BENCHLAB_HZ5_LINUX_MIDFRONT \
    OWNER_FAST_STATE DRAIN_ALL_ON_MISS DRAIN_MASK_ON_MISS \
    DRAIN_MASK_HIT_STOP DRAIN_TAKE_FIRST DRAIN_EMPTY_GATED \
    REMOTE_GLOBAL_RECYCLE OUTBOX_FLUSH_ON_MISS REMOTE_DIRECT_FREE_STATE \
    REMOTE_TRUST_DRAIN_STATE LOOKUP_CACHE
  if [[ "$LINUX_MIDFRONT_REMOTE_OUTBOX" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_MIDFRONT_REMOTE_OUTBOX=1)
    hz5_add_u_define HZ5_MIDFRONT_REMOTE_OUTBOX_SLOTS \
      "$LINUX_MIDFRONT_REMOTE_OUTBOX_SLOTS"
  fi
  if [[ "$LINUX_MIDFRONT_REMOTE_RBUF" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_MIDFRONT_REMOTE_RBUF=1)
    hz5_add_u_define HZ5_MIDFRONT_REMOTE_RBUF_CAP \
      "$LINUX_MIDFRONT_REMOTE_RBUF_CAP"
    hz5_add_u_define HZ5_MIDFRONT_REMOTE_RBUF_FLUSH_THRESHOLD \
      "$LINUX_MIDFRONT_REMOTE_RBUF_FLUSH_THRESHOLD"
  fi
fi
if [[ "$LINUX_LARGEFRONT_L1" -eq 1 ]]; then
  ...
  if [[ "$LINUX_LARGEFRONT_DRAIN_BULK_LOCAL" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_OWNER_INBOX=1)
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_DRAIN_TAKE_FIRST=1)
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_DRAIN_BULK_LOCAL=1)
  fi
  if [[ "$LINUX_LARGEFRONT_DRAIN_TRUST_REMOTE_STATE" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_OWNER_INBOX=1)
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_DRAIN_TAKE_FIRST=1)
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_DRAIN_TRUST_REMOTE_STATE=1)
  fi
  ...
  if [[ "$LINUX_LARGEFRONT_TRANSFER128" -eq 1 ]]; then
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_OWNER_INBOX=1)
    COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_TRANSFER128=1)
    if [[ "$LINUX_LARGEFRONT_TRANSFER128_TLS_FIRST" -eq 1 ]]; then
      COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_TRANSFER128_TLS_FIRST=1)
    fi
    if [[ "$LINUX_LARGEFRONT_TRANSFER128_OWNER_SHARD" -eq 1 ]]; then
      COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_TRANSFER128_OWNER_SHARD=1)
    fi
    if [[ "$LINUX_LARGEFRONT_TRANSFER128_CONSUMER_SHARD" -eq 1 ]]; then
      COMMON_FLAGS+=(-DBENCHLAB_HZ5_LINUX_LARGEFRONT_TRANSFER128_CONSUMER_SHARD=1)
    fi
    COMMON_FLAGS+=(
      -DHZ5_LARGEFRONT_TRANSFER128_CAP="${LINUX_LARGEFRONT_TRANSFER128_CAP}u"
      -DHZ5_LARGEFRONT_TRANSFER128_CHUNK_CAP="${LINUX_LARGEFRONT_TRANSFER128_CHUNK_CAP}u"
      -DHZ5_LARGEFRONT_TRANSFER128_SHARD_COUNT="${LINUX_LARGEFRONT_TRANSFER128_SHARD_COUNT}u"
      -DHZ5_LARGEFRONT_TRANSFER128_SHARD_CAP="${LINUX_LARGEFRONT_TRANSFER128_SHARD_CAP}u"
    )
  fi
