...
void* hz6_front_reuse_or_block_prefill_source_kind(Hz6Allocator* allocator,
                                                   uint16_t front_id,
                                                   uint16_t class_id,
                                                   size_t bytes,
                                                   size_t block_bytes,
  ...
  size_t filled = hz6_front_prefill_source_block_kind(
      allocator, front_id, class_id, bytes, block_bytes, source_kind,
      refill_count);
  if (filled == 0) {
    return hz6_front_reuse_or_prefill_source_kind(
        allocator, front_id, class_id, bytes, source_kind, refill_count);
  }
