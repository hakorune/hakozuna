...
| `hz6-small-boundary-trusted-toy-map8192-external-target` | `build_hz6_preload_small_boundary_trusted_toy_map8192_external_target.sh` | Lower-RSS fixed-boundary profile; latest cross refresh beats tcmalloc balance on fixed rows, but mixed-small rows remain weak. |
