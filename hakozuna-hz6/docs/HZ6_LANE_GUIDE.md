...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, Linux mmap retain, 64K retained stack, ToyFullBlockPrefill max128, route tombstone compact, frontcache bin 8192, PreloadToyActiveFastFree, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M ops/s initially, to about 3.05M after route 131072, to about 7.0M..7.3M after PreloadSourceRetain-L1, then to about 8.2M..10.3M after PreloadSourceRetain64kStack-L1, about 12.3M..12.6M in the first ToyFullBlockPrefill max128 guard, about 17M..18M on the 1M long-run guard after RouteTombstoneCompact, about 22.9M on the 1M frontcache4096 guard, about 34M..35M after frontcache8192, and about 53.9M on the 16..256 activefast cross guard. Keep route 262144, packed-meta, TLS retain, SourceRunReuse, SameOwner, FastFree, frontcache16384, and aggressive tombstone compact as controls, not defaults. |
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Current default bundle: route table 131072, descriptors 32768, source blocks 4096, frontcache bin 8192, Toy active map 32768, mmap retain, 64K retain stack, ToyFull max128, route tombstone compact, xor-fold, linear-wrap, loop-carry. Use this as the canonical Ubuntu LD_PRELOAD lane before trying another knob. |
...
| Ubuntu next target | `MidPage/source path audit` | Frontcache8192 and PreloadToyActiveFastFree close the immediate Toy/preload free pressure. Broader validation now points at the 4096..16384 band: HZ6 beats system/mimalloc there, but tcmalloc remains much faster. The diagnostic row has `midpage_source_alloc=6324`, so continue with MidPage/source retain or prefill shape, not route inline/code-layout knobs. |
