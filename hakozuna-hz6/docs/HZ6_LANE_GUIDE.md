...
| ElasticDescriptorRouteOverflow-L1 | `...elasticdescroute-desc16k-front4k-...source10k-route16k-run512` | Strong RSS evidence, speed no-go/control for now. It shrinks local descriptor and route to `16k`, uses a shared descriptor depot and shared route exact/envelope overflow. Full10k run-1: `33.184M / 246824 KB`, safety clean; main-warmup uses `143664` descriptor depot entries. This proves descriptor overflow works but adds descriptor-storage lookup cost. |
