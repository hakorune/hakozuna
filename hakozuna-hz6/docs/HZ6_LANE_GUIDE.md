...
| Ubuntu target-heavy profile/control | `HZ6_MIDPAGE_DIRECT_LOCAL_SKIP_TRANSFER_FIRST_L1=1` | MidPage direct-local reuse skips transfer-first before the local frontcache pop. Current-shape production repeat is focused-positive but fixed_4k/8k and stats-on speed are weak; keep off selected/default and use `hz6-midpage-skip-transfer-target` only for target-heavy profile checks. |
