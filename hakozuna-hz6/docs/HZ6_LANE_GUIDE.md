...
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Canonical preload lane: route65k, desc16384, source2048, frontcache4096, Toy active map32768, MidPage run256K, MidPage 32K run512K, external MidPage active map16K/probe4 with unaligned slots allowed, MidPage alloc descriptor-out, preload-boundary MidPage malloc skip, realloc in-place, mmap retain, 64K retain stack, ToyFull max128, tombstone compact, xor-fold, linear-wrap, loop-carry. |
...
| Ubuntu selected/default | `HZ6_MIDPAGE_32K_RUN_BYTES=524288` in LD_PRELOAD | MidPage32KRun512-L1. After static table trim, smaller 32K payload-trim runs were no-go; 512K reduced 4096..16384 source churn and repeat-7 moved `42.176M / 94.38 MiB -> 45.298M / 94.50 MiB` while guard rows stayed flat/positive. Keep 256K as direct control. |
