...
| Next design fallback | `ElasticCapacity-L1 unified depot accounting` | Close the slot-owner logical / depot-owner-equal shortcut family as evidence for now. The next useful design should address unified depot accounting, owner-safe drain/localize criteria, or another operation that is not already eliminated by DepotOwnerDirectFastPath-L1. |
