...
| Ubuntu control | `HZ6_MIDPAGE_ACTIVE_MAP_FREE_FAST_SLOT_L1=1` with register fast-slot | Free-side fast-slot remains a control after register fast-slot promotion. `amap_fast_both` was target-positive (`4096..16384 48.584M -> 50.253M`) but weaker on the 1024..4096 guard than register-only, so keep free fast-slot off until a narrower free-hit shape exists. |
