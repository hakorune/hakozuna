...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, Linux mmap retain, 64K retained stack, ToyFullBlockPrefill max128, route tombstone compact, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M ops/s initially, to about 3.05M after route 131072, to about 7.0M..7.3M after PreloadSourceRetain-L1, then to about 8.2M..10.3M after PreloadSourceRetain64kStack-L1, about 12.3M..12.6M in the first ToyFullBlockPrefill max128 guard, and about 17M..18M on the 1M long-run guard after RouteTombstoneCompact. Keep route 262144, packed-meta, TLS retain, SourceRunReuse, SameOwner, FastFree, and aggressive tombstone compact as controls, not defaults. |
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Current default bundle: route table 131072, descriptors 32768, source blocks 4096, frontcache bin 1024, Toy active map 32768, mmap retain, 64K retain stack, ToyFull max128, route tombstone compact, xor-fold, linear-wrap, loop-carry. Use this as the canonical Ubuntu LD_PRELOAD lane before trying another knob. |
...
| Ubuntu next target | `RouteRegister/DescriptorFrontCacheAudit-L1` | After ToyFull and tombstone compact, source syscall churn and full-table route probes are no longer the primary LD_PRELOAD blockers. Cross-allocator 1M still has HZ6 below mimalloc, so next work should split remaining exact route register/unregister cost, descriptor/frontcache mutation cost, and a possible registration design that avoids tombstone accumulation rather than periodically compacting. |
| Ubuntu diagnostic-only | `HZ6_PRELOAD_STATS=1` | Prints one aggregate `[HZ6_PRELOAD_STATS]` line at process unload across registered thread-local preload allocators, including Linux mmap retain hit/miss counters. Use to attribute source/route/fail/retain pressure; do not use for speed ranking. |
