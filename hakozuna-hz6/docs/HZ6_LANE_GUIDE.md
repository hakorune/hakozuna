...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, Linux mmap retain, 64K retained stack, ToyFullBlockPrefill max128, route tombstone compact, frontcache bin 4096, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M ops/s initially, to about 3.05M after route 131072, to about 7.0M..7.3M after PreloadSourceRetain-L1, then to about 8.2M..10.3M after PreloadSourceRetain64kStack-L1, about 12.3M..12.6M in the first ToyFullBlockPrefill max128 guard, about 17M..18M on the 1M long-run guard after RouteTombstoneCompact, and about 22.9M on the 1M frontcache4096 guard. Keep route 262144, packed-meta, TLS retain, SourceRunReuse, SameOwner, FastFree, and aggressive tombstone compact as controls, not defaults. |
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Current default bundle: route table 131072, descriptors 32768, source blocks 4096, frontcache bin 4096, Toy active map 32768, mmap retain, 64K retain stack, ToyFull max128, route tombstone compact, xor-fold, linear-wrap, loop-carry. Use this as the canonical Ubuntu LD_PRELOAD lane before trying another knob. |
...
| Ubuntu selected/default | `HZ6_FRONT_CACHE_BIN_CAPACITY=4096` in LD_PRELOAD | RouteRegister/DescriptorFrontCacheAudit-L1 follow-up. Diagnostic stats showed 1M mixed_ws free-side route lookup is mostly bypassed by Toy active-map, while source-run route registration and frontcache-overflow unregister dominate remaining churn. Raising the preload frontcache bin cap from 1024 to 4096 cut source_alloc from about 21.3K to about 8.7K and improved 1M repeat-3 median from about 18.2M to about 22.9M ops/s without a peak-RSS regression in the guard. |
| Ubuntu next target | `RouteRegister/DescriptorFrontCacheAudit-L1` | The first audit promoted frontcache4096, but exact route register/unregister remains high-volume: source-run slot register and frontcache-overflow unregister still dominate the diagnostic detail lines. Continue with source-run slot registration shape or a narrower retention policy before trying another route inline/code-layout knob. |
