...
| Ubuntu candidate-control | `ToyDirectMapTrusted max4` | Compose direct local alloc/reuse/free, trusted local-cache owner, Toy active free-map, trusted active-map owner, and `HZ6_LOCAL_CACHE_DIRECT_MAX_CLASS=4`. Huge 4K/8K wins after LastHitRouteCache/ToyClassIdFast, but 16K regresses in guard repeat-10, so do not default. |
