...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, Linux mmap retain, 64K retained stack, ToyFullBlockPrefill max128, route tombstone compact, frontcache bin 8192, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M ops/s initially, to about 3.05M after route 131072, to about 7.0M..7.3M after PreloadSourceRetain-L1, then to about 8.2M..10.3M after PreloadSourceRetain64kStack-L1, about 12.3M..12.6M in the first ToyFullBlockPrefill max128 guard, about 17M..18M on the 1M long-run guard after RouteTombstoneCompact, about 22.9M on the 1M frontcache4096 guard, and about 34M..35M after frontcache8192. Keep route 262144, packed-meta, TLS retain, SourceRunReuse, SameOwner, FastFree, frontcache16384, and aggressive tombstone compact as controls, not defaults. |
...
| Ubuntu next target | `RouteRegister/DescriptorFrontCacheAudit-L1` | Frontcache8192 removed the immediate overflow/register churn on the 1M mixed_ws guard and pushed HZ6 above mimalloc in the focused cross check. Continue with cross-row validation and any remaining route lookup cost only after checking the default on broader rows; do not chase route inline/code-layout knobs from the old 4096 pressure profile. |
