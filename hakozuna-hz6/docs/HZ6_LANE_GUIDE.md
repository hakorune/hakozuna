...
| Ubuntu diagnostic-only | `HZ6_PRELOAD_STATS=1` | Prints aggregate `[HZ6_PRELOAD_STATS]`, `[HZ6_PRELOAD_ROUTE_DETAIL]`, `[HZ6_PRELOAD_FRONT_DETAIL]`, `[HZ6_PRELOAD_FRONTCACHE_CLASS_DETAIL]`, and `[HZ6_PRELOAD_PHASE_STATS]` lines at process unload across registered thread-local preload allocators, including Linux mmap retain hit/miss counters and class-level frontcache max/empty counts. Use to attribute source/route/fail/retain/frontcache pressure; do not use for speed ranking. |
