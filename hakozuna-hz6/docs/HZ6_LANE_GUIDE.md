...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, Linux mmap retain, 64K retained stack, ToyFullBlockPrefill max128, route tombstone compact, frontcache bin 8192, MidPage run 256K, MidPage active free map 8K/probe2, PreloadToyActiveFastFree, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M ops/s initially, to about 3.05M after route 131072, to about 7.0M..7.3M after PreloadSourceRetain-L1, then to about 8.2M..10.3M after PreloadSourceRetain64kStack-L1, about 12.3M..12.6M in the first ToyFullBlockPrefill max128 guard, about 17M..18M on the 1M long-run guard after RouteTombstoneCompact, about 22.9M on the 1M frontcache4096 guard, about 34M..35M after frontcache8192, about 53.9M on the 16..256 activefast cross guard, about 19.4M on the 4096..16384 MidPage guard after MidPage run 256K, and about 20.8M on the selected MidPage active-map guard. Keep route 262144, packed-meta, TLS retain, SourceRunReuse, SameOwner, FastFree, frontcache16384, MidPage run 512K, MidPage map 16K, and aggressive tombstone compact as controls, not defaults. |
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Current default bundle: route table 131072, descriptors 32768, source blocks 4096, frontcache bin 8192, Toy active map 32768, MidPage run 256K, MidPage active free map 8K/probe2, mmap retain, 64K retain stack, ToyFull max128, route tombstone compact, xor-fold, linear-wrap, loop-carry. Use this as the canonical Ubuntu LD_PRELOAD lane before trying another knob. |
...
| Ubuntu selected/default | `HZ6_FRONT_CACHE_BIN_CAPACITY=8192` in LD_PRELOAD | RouteRegister/DescriptorFrontCacheAudit-L1 follow-up. Diagnostic stats showed 1M mixed_ws free-side route lookup is mostly bypassed by Toy active-map, while source-run route registration and frontcache-overflow unregister dominate remaining churn. Frontcache4096 cut source_alloc from about 21.3K to about 8.7K and improved the 1M repeat-3 median from about 18.2M to about 22.9M. Frontcache8192 then removed frontcache-overflow unregisters and route tombstones on the diagnostic guard, cut source_alloc to about 541, and raised the focused 1M median to about 34.2M; frontcache16384 was flat, so keep 8192 as the selected boundary. |
...
| Ubuntu selected/default | `HZ6_MIDPAGE_RUN_BYTES=262144` in LD_PRELOAD | MidPageSourceRun256K-L1. The direct/R1 default remains 64KiB, but the preload bundle raises MidPage run bytes to 256KiB to reduce 8K/32K source churn. Repeat-3 on 4096..16384 moved from `16.549M` to `19.394M` median ops/s with flat peak RSS; 512KiB reached `20.181M` but regressed the 16..4096 guard, so keep 512KiB as a MidPage-specialized control. |
| Ubuntu selected/default | `HZ6_MIDPAGE_ACTIVE_FREE_MAP_L2=1` in LD_PRELOAD | Dedicated MidPage active free map. Unlike the no-go Toy-map widening, this map is separate, uses an 8KiB alignment gate, validates descriptor pointer/generation/class/state, and registers MidPage alloc/reuse through exact route descriptors. The selected preload shape uses capacity 8192 and probe limit 2. Focused repeat-3 moved 4096..16384 from `19.383M` to `20.975M`; the post-promotion default guard was `20.762M`. Selected diagnostic route probes dropped to `107375` with `923833` MidPage map hits and zero stale/cache-fail. 16K capacity is a faster MidPage-only control but has a larger allocator-local footprint. |
| Ubuntu diagnostic-only | `HZ6_PRELOAD_STATS=1` | Prints aggregate `[HZ6_PRELOAD_STATS]`, `[HZ6_PRELOAD_ROUTE_DETAIL]`, `[HZ6_PRELOAD_FRONT_DETAIL]`, and `[HZ6_PRELOAD_PHASE_STATS]` lines at process unload across registered thread-local preload allocators, including Linux mmap retain hit/miss counters. Use to attribute source/route/fail/retain/frontcache pressure; do not use for speed ranking. |
