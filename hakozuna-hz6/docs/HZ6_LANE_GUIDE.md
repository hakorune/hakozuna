...
| DepotDescriptorRehomeBudget2048-L1 | `...elasticdescsource-route-depotrunmeta-depotownerdirect-depotdescrehome-budget2048-...source64-route16k-run4096` | Candidate-control for bounded descriptor materialization. It keeps DepotDescriptorRehome-L1 but limits rehome successes to `2048` per allocator. Full10k non-diagnostic is safety-clean at `44.919M`; diagnostic is safety-clean at `40.961M` with `success=30483`, `budget_denied=41328`, `route_replace_fail=0`, `detach_fail=0`, and `rollback=0`. Final `descriptor_used` drops to `36539` and `descriptor_live_max` to `2448`, so this is the current best source-depot follow-up to validate with repeat/guard. |
