...
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Canonical preload lane: route65k, desc16384, source2048, frontcache4096, Toy active map32768, MidPage run256K, MidPage 32K run768K, external MidPage active map16K/probe4 with unaligned slots allowed, MidPage alloc descriptor-out, preload-boundary MidPage malloc skip, realloc in-place, mmap retain, 64K retain stack, ToyFull max128, tombstone compact, xor-fold, linear-wrap, loop-carry. |
...
| Ubuntu selected/default | `HZ6_MIDPAGE_RUN_BYTES=262144` in LD_PRELOAD | MidPageSourceRun256K-L1. The direct/R1 default remains 64KiB, but the preload bundle raises general MidPage run bytes to 256KiB to reduce 8K source churn. 32K now has a class-specific selected run size below. |
| Ubuntu selected/default | `HZ6_MIDPAGE_32K_RUN_BYTES=786432` in LD_PRELOAD | MidPage32KRun768-L1. After run512, a larger 32K run improved the 4096..16384 target with flat RSS: repeat-7 moved `43.110M / 94.50 MiB -> 44.324M / 94.50 MiB`, kept 16..256 and 16..4096 positive, and softened 1024..4096 by about 0.5%. Keep 512K as direct control; 1M/1.5M remain target-positive controls but less guard-clean. |
...
| Ubuntu diagnostic-only | `HZ6_PRELOAD_STATS=1` | Prints aggregate `[HZ6_PRELOAD_STATS]`, `[HZ6_PRELOAD_ROUTE_DETAIL]`, `[HZ6_PRELOAD_FRONT_DETAIL]`, `[HZ6_PRELOAD_FRONTCACHE_CLASS_DETAIL]`, `[HZ6_PRELOAD_PHASE_STATS]`, and `[HZ6_PRELOAD_HOOK_DETAIL]` lines at process unload across registered thread-local preload allocators, including Linux mmap retain hit/miss counters, class-level frontcache max/empty counts, and preload free hook path splits. Use to attribute source/route/fail/retain/frontcache/hook pressure; do not use for speed ranking. |
...
| Ubuntu target-positive control/watch | `HZ6_PRELOAD_FREE_MIDPAGE_CURRENT_BIAS_FIRST_L1=1` | Cheap selective MidPage-first preload free gate using active-map current counts. It cuts target Toy attempts from about 1M to about 2K and improves 4096..16384, but repeat-15 still leaves tiny/mixed guards slightly weak. Ratio follow-up keeps 1x as balanced control/watch; 2x is target upper-bound but guard-negative. |
...
| Ubuntu control/no-go | `HZ6_MIDPAGE_ACTIVE_MAP_FREE_FAST_SLOT_L1=1` | MidPage active-map free lookup tests the base hash slot before the bounded probe loop. It adds no new classifier, but short repeat-5 still regressed the 4096..16384 target (`26.295M -> 25.422M`) and was flat/weak on mixed guards. Keep off. |
...
| Ubuntu control/no-go | `HZ6_MIDPAGE_RUN_BYTES=384K/512K/768K` | 8K source-run widening controls after 32K run768. They reduce source allocation, but the target speed does not translate; keep selected 8K run256K. |
