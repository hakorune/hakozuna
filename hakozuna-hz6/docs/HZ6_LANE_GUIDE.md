...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M to 0.557M ops/s at route 32768, then to about 2.88M ops/s at route 131072 on single-run mixed_ws. Keep route 262144 as upper-bound/control because it did not improve speed and raised RSS. |
