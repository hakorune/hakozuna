...
| Ubuntu target DSO/control | `HZ6_PRELOAD_MIDPAGE_MALLOC_SKIP_TRANSFER_L1=1` | Preload-boundary MidPage malloc skip keeps the selected `hz6_malloc()` shape clean and recovers the strongest target DSO result so far: repeat-7 4096..16384 `34.712M -> 40.501M` with flat RSS. Non-target guards still regress, so this is not selected default. |
