...
| Ubuntu active design | `RemoteRouteOwnerHint-L1` | Current MT remote-route repair phase. Do not treat the debug patch state as selected-final or paper-ready. The active box is to avoid local/visible page-table route scans on definitely-foreign remote frees before rerunning `main_r50`, `main_r90`, and `cross128_r90`. |
