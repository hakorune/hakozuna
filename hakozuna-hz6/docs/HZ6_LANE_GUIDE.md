...
| Next design fallback | `ElasticCapacity-L1 fail-closed descriptor/transfer-local slot witness` | Close the slot-owner logical / depot-owner-equal shortcut family and broad slot-local storage override as evidence for now. The next useful behavior should consume the transfer-scoped slot witness only inside a fail-closed descriptor-local or transfer-local decision; it should not move whole SourceBlocks and should not override general owner_source storage. |
