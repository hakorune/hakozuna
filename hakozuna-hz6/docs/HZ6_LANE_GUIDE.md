...
| Ubuntu profile/control DSO | `build_hz6_preload_midpage_trusted_class_target.sh` | Named MidPage trusted-class local-reuse DSO. It keeps selected default intact and enables only `HZ6_MIDPAGE_DIRECT_LOCAL_REUSE_TRUSTED_CLASS_L1=1` so shared matrices can validate the promising target/fixed win before any selected-default promotion. `HZ6_MIDPAGE_TRUSTED_CLASS_CLASS5_ONLY=1` builds a class5-only split for fixed_16k/profile evidence, not selected default. |
