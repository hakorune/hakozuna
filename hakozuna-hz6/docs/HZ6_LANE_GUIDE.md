...
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Canonical preload lane: route131k, desc32768, source4096, frontcache8192, Toy active map32768, MidPage run256K, external MidPage active map16K/probe4 with unaligned slots allowed, MidPage alloc descriptor-out, realloc in-place, mmap retain, 64K retain stack, ToyFull max128, tombstone compact, xor-fold, linear-wrap, loop-carry. |
...
| Ubuntu selected/default | `HZ6_MIDPAGE_ALLOC_DESCRIPTOR_OUT_L1=1` | MidPage alloc returns the activated descriptor to active-map registration, avoiding the post-alloc exact route lookup without changing prefill policy. Focused repeat-5 improves 4096..16384 strongly and keeps the other preload guards flat/positive, so it is selected for LD_PRELOAD only. |
| Ubuntu selected/default | `HZ6_PRELOAD_REALLOC_IN_PLACE_L1=1` | LD_PRELOAD realloc returns the same pointer when requested size fits the current HZ6 usable descriptor bytes. This removes malloc/copy/free round trips for same-class/shrink reallocs. Repeat-5 versus control-off improved all focused rows, including 4096..16384 `19.971M -> 30.118M`. Latest selected cross repeat-5 now has HZ6 ahead of mimalloc on 16..256, 16..4096, 1024..4096, and 4096..16384; tcmalloc remains the higher throughput ceiling and system remains faster on tiny 16..256. |
...
| Ubuntu control/no-go | `HZ6_MIDPAGE_32K_RUN_BYTES=524288` | Class-specific 32K MidPage run thickening. It cuts source allocation and 32K prefill-run count, but regresses the target 4096..16384 row, so keep the selected 256K shared run default. |
...
| Ubuntu control/no-go | `HZ6_MIDPAGE_ACTIVE_MAP_NO_OVERWRITE_FULL_L1=1` | Collision policy that keeps existing entries when the bounded probe range is full. It regressed the target MidPage row and slightly worsened misses, so keep current overwrite-on-full behavior. |
| Ubuntu control/no-go | `HZ6_MIDPAGE_ACTIVE_MAP_REGISTER_FAST_SLOT_L1=1` | Toy-style register fast-slot for MidPage active-map registration. It worsened selected guards and the target MidPage row, so keep the current loop body. |
