...
For the active MT remote-route cliff design, see
[`HZ6_REMOTE_ROUTE_PHASE_PLAN.md`](HZ6_REMOTE_ROUTE_PHASE_PLAN.md).
...
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Canonical preload lane: route32k, desc8192, source1024, frontcache4096, Toy active map16384, MidPage run768K, MidPage 32K run2048K, external MidPage active map8K/probe4 with unaligned slots allowed, mask-index, register fast-slot, MidPage alloc descriptor-out, preload-boundary MidPage malloc skip, Toy direct-class fast reuse max4096 with boundary trusted-owner, current-bias free order, realloc in-place, mmap retain, 64K retain stack, ToyFull max128, tombstone compact, xor-fold, linear-wrap, loop-carry. |
| Ubuntu/Windows active design | `RemoteFreeRouteResolve-L1` | Current MT remote-route repair phase. `RemoteRouteOwnerHint-L1` is closed as NO-GO because owner-locality is not an ownership proof. The active box is a shared core resolver that separates `PROVEN_EXTERNAL` from `UNRESOLVED_INTEGRITY`, keeps all-visible scan debug-only, and avoids local/visible page-table route scans for proven foreign frees before rerunning `main_r50`, `main_r90`, and `cross128_r90`. |
| Ubuntu selected/default | `HZ6_TOY_SMALL_ACTIVE_FREE_MAP_CAPACITY=16384` + `HZ6_MIDPAGE_ACTIVE_FREE_MAP_CAPACITY=8192` | ActiveMapStorageTrim-L1. After route32K/desc8192/source1024, repeat-9 and follow-up focused/fixed guards cut another `~2 MiB` peak RSS versus the previous Toy32K/MidPage16K map floor with zero fail counters and acceptable speed balance. Keep `map_prev` as direct control. |
...
| Ubuntu control | `HZ6_FRONT_CACHE_BIN_CAPACITY=8192` in LD_PRELOAD | Former wide preload default. StaticTableTrim-L1 promoted frontcache4096, FixedFloorRoute32K-L1 promoted route32K, descriptor/source trim promoted desc8192/source1024, and ActiveMapStorageTrim-L1 promoted Toy16K/MidPage8K maps. Keep 8192, `floor_prev`, and `map_prev` as direct controls. |
...
| Ubuntu selected/default | `HZ6_MIDPAGE_ACTIVE_FREE_MAP_L2=1` + external storage in LD_PRELOAD | Dedicated MidPage active free map. Unlike the no-go Toy-map widening, this map is separate, validates descriptor pointer/generation/class/state, and registers MidPage alloc/reuse through exact route descriptors. The selected preload shape uses external storage, capacity 8192, unaligned slots, and probe limit 4. The old cap16K pass is retained as `map_prev` evidence after active-map storage trim. |
...
| Ubuntu selected API behavior | LD_PRELOAD `malloc_trim(0)` | Explicit quiescent release API. The hook calls `hz6_preload_quiescent_release(0)` to scavenge HZ6 local-free descriptors and flush Linux mmap retained mappings, then forwards to real libc `malloc_trim` when available. It adds no malloc/free hot-path work. No-stats raw `hz6_midpage_payload_trim_ab_20260615_222345` drops current RSS to the `27..28 MiB` floor on focused/fixed rows while peak RSS remains flat. |
| Ubuntu selected/default | `ToyTrustedDefault-L1` | Selected preload now includes `HZ6_PRELOAD_TOY_MALLOC_DIRECT_CLASS_L1=1`, fast reuse, max4096, and `HZ6_PRELOAD_BOUNDARY_TRUSTED_OWNER_L1=1`. Same-run A/B raw `hz6_toy_trusted_default_on_ab_20260616_031042` is large positive on tiny/mid-small/fixed_4k, flat on 4096..16384, and stats-safe in `hz6_toy_trusted_default_on_stats_20260616_031100`. |
...
| Ubuntu diagnostic/control no-go | `HZ6_PAGE_KIND_FREE_SELECTOR_DRYRUN_L1=1` / `HZ6_PAGE_KIND_FREE_SELECTOR_FIRST_L1=1` | Allocator-local page-kind selector populated by active-map registration. It is advisory only and never bypasses descriptor validation or RouteLayer fallback. Dry-run raw `hz6_page_kind_selector_dryrun_20260616_000420` showed zero wrong Toy/MidPage hit classifications, but behavior raw `hz6_page_kind_selector_first_prod_20260616_000902` regressed every focused/fixed row because the all-free lookup/table cost outweighed skipping the wrong first active-map probe. Keep off. |
...
| Ubuntu selected bundle component | `HZ6_PRELOAD_BOUNDARY_TRUSTED_OWNER_L1=1` | This flag is now selected as part of ToyTrustedDefault-L1. Early standalone evidence was mixed, so keep evaluating it through bundle controls rather than reopening the single-flag default decision. |
| Ubuntu control/no-go | `HZ6_DIRECT_LOCAL_FREE_RAW_PUSH_L1=1` | Production-only local-free raw frontcache push. It helps fixed rows, but repeat-15 target guard regressed (`4096..16384 44.689M -> 42.174M`), so keep off unless a profile DSO isolates the fixed-row benefit. |
| Ubuntu historical/profile alias | `build_hz6_preload_toy_target.sh` / `build_hz6_preload_toy_trusted_target.sh` | These builders are now normally selected-equivalent for Toy direct/fast/trusted behavior because ToyTrustedDefault-L1 is selected. Keep aliases for archived matrix compatibility; use `toy_trusted_default_off` for true old-selected A/B. |
| Ubuntu profile/control | `HZ6_PRELOAD_REALLOC_BOUNDARY_ADAPTIVE_4K_L1=1` / `_8K_L1=1` | Adaptive realloc-boundary slack. It arms per-thread only after a boundary realloc copy, then places future exact 4K/8K allocations into the next MidPage slot. After ToyTrustedDefault-L1 it remains profile/control: fixed-boundary rows improve, but mixed/fixed guard regressions prevent selected/default promotion. |
| Ubuntu profile/control DSO | `build_hz6_preload_small_boundary_fast_target.sh` | Named small/fixed-boundary fast DSO. It keeps selected default intact and combines Toy direct fast reuse, realloc-boundary slack, trusted-owner preload boundary, and raw frontcache push. Repeat-15 no-stats raw `hz6_midpage_payload_trim_ab_20260616_002519` improved tiny/focused/fixed rows with near-flat RSS and stats safety `fail=0`; use `hz6-small-boundary-fast-target` only for profile comparisons. |
| Ubuntu profile/control DSO | `build_hz6_preload_small_boundary_trusted_target.sh` | Preferred broad small/fixed-boundary profile DSO. It keeps selected default intact and combines the small-boundary profile with trusted-owner preload boundary while leaving raw-push off. Latest focused/fixed repeats prefer `hz6-small-boundary-trusted-target` over the fast/raw-push sibling for broad profile use; keep selected/default unchanged. |
| Ubuntu selected alias/control DSO | `build_hz6_preload_midpage_trusted_class_target.sh` | Historical MidPage trusted-class local-reuse DSO. The trusted-class lane is now selected in Ubuntu preload, so this builder normally matches selected behavior; `HZ6_MIDPAGE_TRUSTED_CLASS_CLASS5_ONLY=1` still builds a class5-only split for fixed_16k/profile evidence, not selected default. |
...
| Ubuntu control/no-go | Toy active-map full mask-wrap | The `toy_mask_index` runner control now masks probe wrap as well as the base index. Raw `hz6_toy_mask_wrap_ab_20260616_001459` stayed mixed and regressed mid-small/target guards, so selected Toy active-map wrap remains unchanged. |
...
| Ubuntu control/watch | `HZ6_MIDPAGE_DIRECT_LOCAL_SKIP_TRANSFER_FIRST_L1=1` | MidPage direct-local reuse skips transfer-first before the local frontcache pop. Current-shape isolated repeat had some focused positives, but the profile frontier regressed 16..4096 and 4096..16384; keep off selected/default and use `hz6-midpage-skip-transfer-target` only for explicit checks. |
