...
| Next diagnostic target | `FreeLocalCacheOwnerPredicate-L0` | OwnerEqualCallsiteDryRun-L1 shows owner-equality pressure is dominated by free/local-cache. The next short experiment should classify those free/local-cache owner_equal calls by cheap object state/source-depot predicates before any sparse slot-owner probing. |
