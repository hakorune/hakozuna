...
| Next design fallback | `ElasticCapacity-L1 transfer-scoped depot slot drain` | Close the slot-owner logical / depot-owner-equal shortcut family and broad slot-local storage override as evidence for now. The next useful behavior should keep slot-localization scoped to transfer reuse or another fail-closed descriptor-local path; it should not move whole SourceBlocks and should not override general owner_source storage. |
