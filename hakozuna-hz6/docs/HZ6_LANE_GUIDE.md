...
| Ubuntu control/no-go | `HZ6_PRELOAD_BOUNDARY_TRUSTED_OWNER_L1=1` | Preload Toy/MidPage boundary malloc skips duplicate owner-liveness checks after selecting the trusted owner allocator. It improved some target/fixed rows, but repeat-15 tiny guard regressed `16..256 56.457M -> 54.229M`; keep off for selected default. |
...
| Ubuntu profile/control DSO | `build_hz6_preload_toy_trusted_target.sh` | Named Toy/mid-small trusted-owner DSO. It adds `HZ6_PRELOAD_BOUNDARY_TRUSTED_OWNER_L1=1` to the Toy target while leaving realloc-boundary slack off. Decomposition raw `hz6_trusted_profile_decomp_refresh_20260616_024517` makes it the lighter profile candidate for mixed-small, target, and fixed_16k; keep selected/default unchanged pending full guard refresh. |
