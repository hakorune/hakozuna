...
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Canonical preload lane: route131k, desc32768, source4096, frontcache8192, Toy active map32768, MidPage run256K, external MidPage active map8K/probe4 with unaligned slots allowed, realloc in-place, mmap retain, 64K retain stack, ToyFull max128, tombstone compact, xor-fold, linear-wrap, loop-carry. |
| Ubuntu target lane | `MidPageHz4Close-L1` | Current development target for LD_PRELOAD. HZ6 is near HZ4 on `4096..16384` (`29.409M` vs `31.089M`) and has lower RSS than HZ4 in the latest guard. First diagnose MidPage active-map hit/miss/cache-fail and route fallback pressure, then thin the confirmed hot edge. |
