...
| Next behavior target | `DepotDescriptorOwnerEqualFastPath-L1` | Try one narrow behavior only: in free/local-cache owner equality, when `hz6_allocator_descriptor_is_depot()` is true, compare against depot owner metadata without sparse slot-owner probing. Miss/non-depot remains on the normal owner path. |
| Next design fallback | `ElasticCapacity-L1 unified depot accounting` | If the free/local-cache predicate is weak, close the slot-owner logical fast-path family as evidence and return to unified depot accounting, owner-safe drain/localize criteria, and promotion rules for whether the lower-RSS source-depot shape can recover ElasticDescriptorRouteOverflow-class throughput. |
