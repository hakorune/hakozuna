...
| Next design fallback | `ElasticCapacity-L1 slot-level depot drain/localize` | Close the slot-owner logical / depot-owner-equal shortcut family as evidence for now. The next useful behavior should localize slot ownership/storage state for depot transfer reuse; it should not move whole SourceBlocks and should not add another owner_equal branch already covered by DepotOwnerDirectFastPath-L1. |
