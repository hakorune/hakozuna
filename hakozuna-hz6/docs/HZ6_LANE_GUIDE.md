...
| Selected Larson Elastic low-RSS sibling | `...depotownerdirect...` | Best source-depot speed/RSS shape so far. It bypasses OwnerSourceSideMeta-L2 for depot descriptors by reading/writing the depot owner table directly. Repeat-3 guard against the packed source10k sibling is safety-clean, average `+0.52%` speed with about `-195 MB` peak RSS. Selected as a Larson/Elastic low-RSS sibling, not a broad HZ6 default. |
...
| DescriptorDepotOwnerDirectFastPath-L1 | `...elasticdescsource-route-depotownerdirect-...source64-route16k-run512` | Selected Larson Elastic low-RSS sibling after SlotOwnerSparseMeta-L1. Depot descriptors read/write the shared depot owner table before OwnerSourceSideMeta-L2 storage lookup. Repeat-3 guard versus the packed source10k sibling: average `+0.52%`, min `-1.81%`, about `187-199 MB` lower RSS, safety clean. Keep as the current best source-depot shape; not broad default. |
