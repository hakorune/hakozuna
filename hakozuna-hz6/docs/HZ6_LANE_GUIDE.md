...
| Candidate-watch/control | `toysmallactivemap-directlocalfreereuse-largerlowrss-front8k-sourcerun-desc8k-route8k` | Toy/small active free-map over DirectLocalFreeReuse. It keeps a bounded per-allocator active pointer map so same-owner active frees can bypass exact-route lookup and fall back to route lookup on miss/stale/cache-fail. Probe4 repeat-3 is safety-clean and improves 512B/1K/2K, but loses 256B slightly and adds extra active-map state, so keep as control evidence rather than default. |
