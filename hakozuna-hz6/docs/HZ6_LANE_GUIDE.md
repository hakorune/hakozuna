...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. This lane is smoke-clean and useful for cross-allocator harness work, but it is not a promoted HZ6 performance row yet; current mixed_ws preload throughput is dominated by preload ownership/route overhead. |
