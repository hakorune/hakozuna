...
| Ubuntu functional lane | `hz6` LD_PRELOAD allocator | Linux compare-runner integration for HZ6 through `hakozuna-hz6/out/linux/hz6_preload/libhakozuna_hz6_preload.so`. Build with `./hakozuna-hz6/linux/build_hz6_preload.sh` or include `hz6` in `run_linux_bench_compare_matrix.sh --allocators`. The preload build uses larger compare-lane capacity defaults, route table 131072, Linux mmap retain, 64K retained stack, ToyFullBlockPrefill max128, route tombstone compact, frontcache bin 8192, MidPage run 256K, external MidPage active free map 8K/probe2, PreloadToyActiveFastFree, and route xor-fold/linear-wrap/loop-carry. It is smoke-clean and improved from about 0.236M ops/s initially, to about 3.05M after route 131072, to about 7.0M..7.3M after PreloadSourceRetain-L1, then to about 8.2M..10.3M after PreloadSourceRetain64kStack-L1, about 12.3M..12.6M in the first ToyFullBlockPrefill max128 guard, about 17M..18M on the 1M long-run guard after RouteTombstoneCompact, about 22.9M on the 1M frontcache4096 guard, about 34M..35M after frontcache8192, about 53.9M on the 16..256 activefast cross guard, about 19.4M on the 4096..16384 MidPage guard after MidPage run 256K, and about 19.9M on the external MidPage active-map repeat-7 guard. Keep route 262144, packed-meta, TLS retain, SourceRunReuse, SameOwner, FastFree, frontcache16384, MidPage run 512K, internal MidPage map, MidPage map 16K, and aggressive tombstone compact as controls, not defaults. |
| Ubuntu LD_PRELOAD default bundle | `build_hz6_preload.sh` default flags | Current default bundle: route table 131072, descriptors 32768, source blocks 4096, frontcache bin 8192, Toy active map 32768, MidPage run 256K, external MidPage active free map 8K/probe2, mmap retain, 64K retain stack, ToyFull max128, route tombstone compact, xor-fold, linear-wrap, loop-carry. Use this as the canonical Ubuntu LD_PRELOAD lane before trying another knob. |
...
| Ubuntu selected/default | `HZ6_MIDPAGE_ACTIVE_FREE_MAP_L2=1` + external storage in LD_PRELOAD | Dedicated MidPage active free map. Unlike the no-go Toy-map widening, this map is separate, uses an 8KiB alignment gate, validates descriptor pointer/generation/class/state, and registers MidPage alloc/reuse through exact route descriptors. The selected preload shape uses external storage, capacity 8192, and probe limit 2. Repeat-7 versus no-map control moved 1024..4096 from `30.962M` to `32.011M` and 4096..16384 from `18.983M` to `19.903M`. Internal storage is stronger on MidPage-only (`20.504M`) but loses the balanced locality tradeoff, so keep it as a control. |
