...
| `HZ6_PRELOAD_FREE_MIDPAGE_CURRENT_BIAS_FIRST_L1=1` | target-positive control/watch | Cheap selective MidPage-first free gate using allocator-local active-map current counts. If MidPage active entries exceed Toy active entries, free tries MidPage first. Repeat-15 improved 4096..16384 but left small guards slightly weak, so keep off pending broader confirmation. |
