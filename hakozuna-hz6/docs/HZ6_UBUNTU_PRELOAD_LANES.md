...
| `HZ6_PRELOAD_CALLOC_REAL_FALLBACK_L1=1` | control/no-go | Default-off calloc control and audit runner. The control delegates preload `calloc` to the next real calloc implementation instead of HZ6 malloc plus memset. Smoke raw `hz6_preload_calloc_audit_smoke3_20260616_034054` rejects standalone fallback: selected keeps `calloc4k` around `24.8M ops/s` and `calloc32k` around `13.3M ops/s`, while real-calloc fallback is only about `17.5K ops/s` because subsequent `free()` goes through HZ6 route miss before real free. Do not default or profile this without a paired real-pointer free-skip design. |
