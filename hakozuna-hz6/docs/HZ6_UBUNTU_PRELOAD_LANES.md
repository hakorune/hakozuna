...
| `HZ6_MIDPAGE_DIRECT_LOCAL_REUSE_TRUSTED_CLASS_L1=1` | promising control, not default yet | Default-off MidPage preload-boundary local-reuse success-path shape. It assumes the caller already selected a valid MidPage class and passes a descriptor out pointer, but preserves descriptor class/generation/state activation validation. Repeat-15 production raw `hz6_midpage_payload_trim_ab_20260616_004406` was positive on target/fixed without RSS growth: `4096..16384 44.338M -> 45.576M`, `fixed_4k 31.693M -> 32.163M`, `fixed_8k 41.861M -> 43.045M`, `fixed_16k 44.036M -> 45.256M`, and `1024..4096 33.424M -> 33.752M`; tiny was flat, but `16..4096` slightly regressed (`36.187M -> 35.978M`). Stats raw `hz6_midpage_payload_trim_ab_20260616_004427` kept fail=0 and RSS/payload stable, but diagnostics shape is mixed. Shared alias raw `hz6_ubuntu_selected_balance_20260616_004725` was positive on focused rows with equal/slightly lower RSS, but `hz6_ubuntu_size_slices_20260616_004747` regressed `fixed_8k` (`33.164M -> 32.558M`). Keep default-off pending broader guard confirmation. Shared compare alias: `hz6-midpage-trusted-class` / `hz6_midpage_trusted_class`. |
...
| `build_hz6_preload_midpage_trusted_class_target.sh` | Named MidPage trusted-class local-reuse profile DSO. It keeps selected default intact and enables only `HZ6_MIDPAGE_DIRECT_LOCAL_REUSE_TRUSTED_CLASS_L1=1`. Shared compare aliases: `hz6-midpage-trusted-class` / `hz6_midpage_trusted_class`. |
