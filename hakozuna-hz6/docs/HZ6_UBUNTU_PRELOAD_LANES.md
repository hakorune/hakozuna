...
| Toy target DSO | profile/control | `./hakozuna-hz6/linux/build_hz6_preload_toy_target.sh` builds `out/linux/hz6_preload_toy_target/libhakozuna_hz6_preload.so` with the selected bundle plus `HZ6_PRELOAD_TOY_MALLOC_DIRECT_CLASS_L1=1` and max4096. Use this when the workload is known to be Toy/mid-small heavy. It is intentionally separate from selected default because target/fixed guards rejected default promotion. Production repeat-7 raw `hz6_midpage_payload_trim_ab_20260615_220021` confirms the split: `16..256 56.457M -> 60.441M`, `16..4096 35.364M -> 37.573M`, `1024..4096 33.455M -> 34.507M`, and `fixed_4k 31.632M -> 33.147M`, while `4096..16384`, `fixed_8k`, and `fixed_16k` regress. |
...
| LD_PRELOAD `malloc_trim` hook | selected API/control behavior | Added `malloc_trim(size_t pad)` interposition. It scavenges HZ6 local-free descriptors, then forwards to real libc `malloc_trim` if present. This exposes the quiescent RSS recovery through a standard opt-in API without adding malloc/free hot-path work. Stats-on raw `hz6_midpage_payload_trim_ab_20260615_211643` matched direct scavenge: 4096..16384 current RSS `94.12 -> 70.93 MiB`, fixed_16k `93.25 -> 59.80 MiB`, payload attribution after trim about `0.25 MiB`. No-stats raw `hz6_midpage_payload_trim_ab_20260615_211713` confirmed current RSS recovery while peak RSS stayed flat. |
