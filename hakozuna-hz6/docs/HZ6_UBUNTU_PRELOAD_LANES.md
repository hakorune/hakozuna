...
| Next lane | diagnostic/design | FixedCostResidencyMatrix-L1. Re-audit HZ6 fixed RSS cost against HZ3/HZ4/tcmalloc by splitting touched MidPage payload, frontcache/static tables, active-map storage, and source-run residency. Do not reopen cold-retire behavior, active-map widening, PageKind/free-order tables, or default realloc-boundary behavior without new diagnostic evidence. |
...
| Small-boundary trusted Toy-map8192 target DSO | fixed-boundary RSS profile/control, no default | `./hakozuna-hz6/linux/build_hz6_preload_small_boundary_trusted_toy_map8192_target.sh` combines the preferred small-boundary trusted profile with `HZ6_TOY_SMALL_ACTIVE_FREE_MAP_CAPACITY=8192`. Production repeat-9/no-stats raw `hz6_midpage_payload_trim_ab_20260616_062305` shows the intended tradeoff versus `small_boundary_trusted`: `fixed_4k 44.441M / 79.62 MiB -> 44.792M / 78.75 MiB`, `fixed_8k 43.318M / 80.12 MiB -> 43.797M / 79.12 MiB`, `fixed_16k 43.276M / 80.00 MiB -> 43.362M / 79.25 MiB`, and `4096..16384 42.622M / 81.25 MiB -> 43.002M / 80.25 MiB`. Fixed-frontier repeat-5 raw `hz6_fixed_boundary_profile_frontier_20260616_063106` makes it the best fixed-boundary ops/MiB profile in the current HZ6 set: `fixed_4k 39.262M / 78.62 MiB`, `fixed_8k 37.194M / 79.12 MiB`, `fixed_16k 37.366M / 79.25 MiB`. Cross-allocator fixed-mid raw `hz6_ubuntu_size_slices_20260616_063423` positions it as the measured speed leader on `fixed_16k` (`36.575M / 79.38 MiB`), while HZ3 remains stronger on `fixed_4k`/`fixed_8k` (`41.737M / 68.50 MiB`, `37.868M / 69.88 MiB`). On `fixed_8k`, HZ6 still beats tcmalloc/HZ4/mimalloc speed. Stats-on safety raw `hz6_midpage_payload_trim_ab_20260616_063240` keeps `fail=0`. It regresses mixed-small guards (`16..4096 40.224M -> 39.563M`, `1024..4096 36.351M -> 35.789M`), so keep it out of selected/default and use it only when fixed-boundary RSS/floor matters more than broad mixed-small speed. Aliases: `hz6-small-boundary-trusted-toy-map8192-target` / `hz6_small_boundary_trusted_toy_map8192_target`. |
| Small-boundary trusted Toy-map8192 external/packed combos | runner-only RSS controls, no default/profile alias | Added runner-only variants `small_boundary_trusted_toy_map8192_external`, `small_boundary_trusted_toy_map8192_frontcache_packed`, and `small_boundary_trusted_toy_map8192_external_frontcache_packed` to test whether the fixed-boundary RSS profile should absorb Toy external active-map storage or packed frontcache metadata. Production repeat-5/no-stats raw `hz6_midpage_payload_trim_ab_20260616_071900` shows the RSS direction is real: versus `small_boundary_trusted_toy_map8192`, `external` lowers fixed peaks by about `1.0..1.1 MiB` (`fixed_4k 78.75 -> 77.75 MiB`, `fixed_8k 79.38 -> 78.25 MiB`, `fixed_16k 79.25 -> 78.25 MiB`) and helps target RSS (`4096..16384 79.62 -> 78.88 MiB`). Speed is mixed: `external` helps `4096..16384` but weakens `fixed_8k`/`fixed_16k` in the production repeat; `frontcache_packed` preserves fixed_4k/8k speed better but has weaker RSS; combined external+packed gets the lowest RSS but still mixed speed. Stats+diagnostic raw `hz6_midpage_payload_trim_ab_20260616_071929` kept `fail=0`. Keep these as runner-only controls; do not add a persistent DSO alias unless a future fixed-frontier repeat shows a stable ops/MiB win over the existing Toy-map8192 profile. |
...
| `FixedQuiescentRssMatrix-L1` | diagnostic/current-RSS lane | Added `linux/run_hz6_fixed_quiescent_rss_matrix.sh` plus RSS-profile variants `small_boundary_trusted_toy_map8192_scavenge_before_rss` and `small_boundary_trusted_toy_map8192_malloc_trim_before_rss`. This keeps fixed_4k/8k/16k current-RSS trim checks separate from peak-RSS/cross-allocator ranking. Raw `hz6_fixed_quiescent_rss_matrix_20260616_064654` confirms peak RSS stays at about `79..80 MiB`, while current RSS after `malloc_trim` drops to about `15.1 MiB` for selected and `14.4..14.6 MiB` for the RSS profile. Read rule: peak RSS remains the high-water guard; current RSS tells whether quiescent release returns resident pages for workloads that explicitly trim. |
...
| `run_hz6_preload_profile_frontier.sh` | Thin orchestration runner for the HZ6 profile frontier. It builds only HZ6 selected, requested profile aliases, and the shared benchmark, then reuses the selected-balance and size-slice matrices with child rebuilds disabled. Use it to refresh focused/fixed/large-span profile position from one entrypoint without changing selected/default flags. Default profile set includes toy-trusted, small-boundary-trusted, split realloc-boundary, adaptive realloc-boundary, aligned, calloc-direct, calloc-real, and calloc-large-real aliases. MidPage skip-transfer is available by explicit alias only after raw `hz6_preload_profile_frontier_20260616_065739` rejected default inclusion. |
...
| `run_hz6_midpage_payload_trim_ab.sh` | Focused A/B runner for 32K run-size payload/supply controls, including the run2048 fine ladder and phase-count compile-out controls. Supports `--variants`, `--include-tiny`, `--stats`, and `--no-stats`; use `--no-stats` for speed/RSS promotion gates and `--stats` for fail/source attribution. Short variant aliases include `midpage_skip_transfer_first`, `small_boundary_fast` for `small_boundary_raw_push_trusted_owner`, `small_boundary_trusted` for `small_boundary_trusted_owner`, `small_boundary_trusted_fused` for the trusted profile plus fused MidPage boundary helper, and `small_boundary_trusted_toy_map8192` for the fixed-boundary RSS control. Smoke raw `hz6_small_boundary_trusted_runner_alias_smoke_20260616_024030` confirms the trusted alias. |
