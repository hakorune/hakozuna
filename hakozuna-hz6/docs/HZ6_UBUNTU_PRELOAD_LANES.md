...
this short repeat. `alloc_fail=0` throughout, so this is not a capacity failure
...
| `run_hz6_workload_capacity_shape_sweep.sh` | Diagnostic shape-sweep runner for workload-capacity profiles. It varies working-set slots and size bands around the current proxy rows, compares capacity-narrow/capacity-hybrid by default, and summarizes speed/RSS/ops-per-MiB winners. Use it to find row-shape boundaries before changing workload recommendations; it is proxy evidence only and not selected/default promotion evidence. Smoke raw `hz6_workload_capacity_shape_sweep_20260616_112802` confirms wiring; short repeat raw `112816` shows shape-dependent wins with no allocation failures. |
