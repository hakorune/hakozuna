...
| `run_hz6_workload_proxy_matrix.sh` | Workload-proxy guard runner over `bench_mixed_ws_crt`. It compares selected HZ6, the preferred `hz6-workload-capacity-lite-target`, fixed/RSS profiles, HZ3/HZ4, mimalloc, tcmalloc, and system on `redis_proxy`, small-object cache, mixed-object cache, and MidPage cache proxy rows. Use it to catch live-working-set capacity cliffs before changing selected/default; these rows are proxy evidence, not app benchmarks. Raw `hz6_workload_proxy_matrix_20260616_075255` found selected desc8192 collapse, raw `075550` confirmed full-capacity recovery, and raw `080227` selected the lighter `hz6-workload-capacity-lite-target` as the preferred default runner profile. Use `hz6-workload-capacity-mid-target` or `hz6-workload-capacity-target` explicitly when testing larger controls. |
