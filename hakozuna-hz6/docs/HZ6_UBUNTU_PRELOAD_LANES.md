...
| Next lane | diagnostic/design | Cold source-block retire, current-bias predicate variants, and active-map collision/layout are closed as controls/no-go. PageKindFreeSelector dry-run is the current free-order follow-up because it predicts Toy vs MidPage pages without changing RouteLayer behavior. |
...
| `HZ6_PAGE_KIND_FREE_SELECTOR_DRYRUN_L1=1` | diagnostic/design | Allocator-local advisory page-kind table populated by Toy/MidPage active-map registration. Preload free only compares the prediction with actual active-map hits; it never manufactures a route result or skips descriptor validation. Raw `private/raw-results/linux/hz6_page_kind_selector_dryrun_20260616_000420` showed zero wrong Toy/MidPage hit classifications across focused+fixed repeat-3 (`16..4096`, `1024..4096`, `4096..16384`, fixed 4K/8K/16K), but the dry-run lookup/counters are too expensive for throughput. Next step is a behavior A/B that uses toy/midpage predictions to skip the wrong first active-map probe and leaves unknown/mixed/stale cases on selected current-bias order. |
