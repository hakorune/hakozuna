...
| `run_hz6_preload_profile_frontier.sh` | Thin orchestration runner for the HZ6 profile frontier. It reuses the selected-balance and size-slice matrices with HZ6 selected plus named profile aliases, so focused/fixed/large-span profile position can be refreshed from one entrypoint without changing selected/default flags. |
