...
| `run_hz6_workload_profile_guard.sh` | Narrow workload-profile decision guard. It wraps `run_hz6_workload_proxy_matrix.sh` with selected/default, workload-capacity-narrow, workload-descriptor-hybrid, the route16K fixed profile, and external allocators. Use it to refresh application-profile recommendations without changing selected/default. Thin raw `hz6_workload_profile_guard_20260616_105102` keeps capacity-narrow and descriptor-hybrid paired: both recover large live-set small/mixed/wide cache proxy rows by orders of magnitude, both beat tcmalloc on `wide_midpage_cache` in that repeat, and route16K remains a fixed/redis/midpage-leaning profile rather than the workload-capacity answer. |
...
| `run_hz6_workload_profile_gap_diag.sh` | Diagnostic runner for the current explicit workload controls. It builds selected, workload-capacity-narrow, and descriptor-hybrid with stats/diagnostic probes, then summarizes descriptor exhaustion, elastic descriptor use, route probes, source blocks, static bytes, payload bytes, and attributed bytes. Raw `hz6_workload_profile_gap_diag_20260616_093918` shows capacity-narrow and descriptor-hybrid are counter-identical on the collapsed rows at 20K iterations, with `elastic_alloc=0`. |
