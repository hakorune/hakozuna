...
OUTDIR="${OUTDIR:-${HZ6_DIR}/private/raw-results/linux/hz6_transfer_shard_policy_ab_$(date +%Y%m%d_%H%M%S)}"
...
usage() {
  cat <<'EOF'
Usage:
  ./hakozuna-hz6/linux/run_hz6_preload_transfer_shard_policy_ab.sh [options]

Options:
  --runs N       repeat count per row/variant (default: 3)
...
if [[ ! -x "$BENCH" ]]; then
  echo "[hz6][transfer-shard-policy] missing bench: $BENCH" >&2
  exit 2
fi
if [[ ! -x /usr/bin/time ]]; then
  echo "[hz6][transfer-shard-policy] missing /usr/bin/time" >&2
  exit 2
fi
...
for variant in "${variants[@]}"; do
  echo "[hz6][transfer-shard-policy] building ${variant}" >&2
  ...
  for row in "${rows[@]}"; do
    ...
    for run in $(seq 1 "$RUNS"); do
      ...
      echo "[hz6][transfer-shard-policy] ${variant} ${name} run=${run} status=${status} ops/s=${ops:-NA} peak_kb=${peak:-NA}" >&2
...
print("\t".join(["variant", "row", "run"] + counters))
with open(sys.argv[1], newline="") as f:
    reader = csv.DictReader(f, delimiter="\t")
    for row in reader:
