...
ITERS="${ITERS:-300000}"
WS="${WS:-4096}"
ALLOCATORS="${ALLOCATORS:-system,hz3,hz4,hz6,mimalloc,tcmalloc}"
ROWS_CSV="${ROWS:-fixed_mid}"
OUTDIR="${OUTDIR:-${ROOT_DIR}/hakozuna-hz6/private/raw-results/linux/hz6_ubuntu_size_slices_$(date +%Y%m%d_%H%M%S)}"
SKIP_BUILDS=0
...
usage() {
  cat <<'EOF'
Usage:
  ./hakozuna-hz6/linux/run_hz6_ubuntu_size_slices_matrix.sh [options]
...
print("# HZ6 Ubuntu Size Slices Matrix\n")
print(f"root: `{root}`\n")
