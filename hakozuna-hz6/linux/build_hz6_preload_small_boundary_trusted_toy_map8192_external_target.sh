#!/usr/bin/env bash
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
HZ6_DIR="${ROOT_DIR}/hakozuna-hz6"

source "${HZ6_DIR}/linux/hz6_preload_profile_builder.sh"

HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS=()
hz6_preload_profile_selected_cflags \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS 1
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_PRELOAD_TOY_MALLOC_DIRECT_CLASS_L1 1
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_PRELOAD_TOY_MALLOC_DIRECT_CLASS_FAST_REUSE_L1 1
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_PRELOAD_TOY_MALLOC_DIRECT_CLASS_MAX_BYTES 4096
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_PRELOAD_REALLOC_BOUNDARY_SLACK_L1 1
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_PRELOAD_BOUNDARY_TRUSTED_OWNER_L1 1
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_TOY_SMALL_ACTIVE_FREE_MAP_CAPACITY 8192
hz6_preload_replace_define \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  HZ6_TOY_SMALL_ACTIVE_FREE_MAP_EXTERNAL_L1 1

hz6_preload_profile_build \
  HZ6_SMALL_BOUNDARY_TRUSTED_TOY_MAP8192_EXTERNAL_TARGET_CFLAGS \
  "${ROOT_DIR}/hakozuna-hz6/out/linux/hz6_preload_small_boundary_trusted_toy_map8192_external_target"
