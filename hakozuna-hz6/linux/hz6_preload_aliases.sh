...
hz6_preload_build_requested_aliases() {
  ...
  if hz6_preload_allocator_requested "$allocators_csv" \
      ...
      "hz6_high_remote_owner_inbox_direct_reuse_target"; then
    "${root_dir}/hakozuna-hz6/linux/build_hz6_preload_high_remote_owner_inbox_direct_reuse_target.sh"
  fi
  if hz6_preload_allocator_requested "$allocators_csv" \
      "hz6-high-remote-transfer-presence-target" \
      "hz6_high_remote_transfer_presence_target"; then
  ...
  if hz6_preload_allocator_requested "$allocators_csv" \
      ...
      "hz6_cross128_toy2_route_before_maps_target"; then
    "${root_dir}/hakozuna-hz6/linux/build_hz6_preload_cross128_toy2_route_before_maps_target.sh"
  fi
  if hz6_preload_allocator_requested "$allocators_csv" \
  ...
  if hz6_preload_allocator_requested "$allocators_csv" \
      ...
      "hz6_transfer_small_class_shard_target"; then
    "${root_dir}/hakozuna-hz6/linux/build_hz6_preload_transfer_small_class_shard_target.sh"
  fi
  if hz6_preload_allocator_requested "$allocators_csv" \
