...
static void* producer_main(void* arg) {
  ...
  for (uint64_t i = 0; i < q->iters; ++i) {
    void* ptr = NULL;
    int rc = posix_memalign(&ptr, q->align, q->size);
    if (rc != 0 || !ptr) {
      fprintf(stderr, "producer posix_memalign failed rc=%d iter=%llu\n", rc,
...
int main(int argc, char** argv) {
  ...
  if (iters == 0 || size == 0 || align == 0 ||
    ...
    fprintf(stderr,
            "usage: bench_remote64k [iters] [size] [align] "
  ...
  printf("allocator=preload mode=producer-consumer iters=%llu "
