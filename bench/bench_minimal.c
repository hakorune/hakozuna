// Minimal hz3 bench (Windows-friendly)
// Usage: bench_minimal [threads] [iters_per_thread] [min_size] [max_size]
...
static unsigned __stdcall bench_thread(void* arg) {
    ...
    size_t span = (ta->max_size > ta->min_size) ? (ta->max_size - ta->min_size + 1) : 1;

    for (size_t i = 0; i < ta->iters; i++) {
        size_t size = ta->min_size + (lcg_next(&seed) % span);
...
int main(int argc, char** argv) {
    ...
    if (argc > 3) min_size = (size_t)strtoull(argv[3], NULL, 10);
    if (argc > 4) max_size = (size_t)strtoull(argv[4], NULL, 10);
    ...
    printf("threads=%zu iters=%zu size=%zu..%zu time=%.3f ops/s=%.3f\n",
           threads, iters, min_size, max_size, sec, ops_sec);
