...
int main(int argc, char** argv) {
  ...
  if (prelude_blocks == 0 || prelude_rounds <= 0 || measure_iters == 0 ||
    ...
    fprintf(stderr,
            "usage: bench_mixed_prelude [prelude_blocks] [prelude_rounds] "
            "[measure_iters] [size] [align] [probe_size] [probe_align] "
            "[probe_attempts]\n");
    return 2;
  }
  ...
  int status =
      run_plateau(prelude_blocks, prelude_rounds, size, align, &prelude);
  if (status != 0) {
    return status;
  }
  ...
  status = run_plateau(probe_attempts, 1, probe_size, probe_align, &probe);
  if (status != 0) {
    return status;
  }
  ...
  for (uint64_t i = 0; i < measure_iters; ++i) {
    ...
    int rc = posix_memalign(&ptr, align, size);
    if (rc != 0 || !ptr) {
      fprintf(stderr, "measure posix_memalign failed rc=%d iter=%llu\n", rc,
  ...
  printf("allocator=preload mode=mixed-prelude prelude_blocks=%zu "
         "prelude_rounds=%d measure_iters=%llu size=%zu align=%zu "
         "probe_size=%zu probe_align=%zu probe_attempts=%zu "
         "probe_successes=%llu probe_nulls=0 time=%.6f ops/s=%.3f "
         ...
         (unsigned long long)probe.allocs, elapsed, (double)measure_ops / elapsed,
