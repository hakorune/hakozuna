...
Keep `active_spans[]` as a weak hint, not as ownership truth.

`OwnerAdmissionReuseSafety-L1` is closed by packing lifecycle refs into the
...
Remote collect optimization details are split into
[`HZ8_REMOTE_COLLECT.md`](HZ8_REMOTE_COLLECT.md).  That document owns the
pending carry, bitmap word drain, pending word summary, bulk commit, RSS
discard, and release benchmark lanes.
