...
Status: **recorded / local code-shape boxes promoted**.

Behavior was unchanged. This note keeps the local-row attribution summary and
points the detailed A/B logs to the benchmark artifacts.
...
The same-run matrix mixed different worker shapes under the same `local0`
label.
...
iteration when `remote_pct=0`. That means guard/local0 cannot be compared
directly with main/medium local0 without accounting for worker tax.
...
The next suspect is medium entry / identity / class-switch shape rather than
...
```text
broad owner-token inline:
  NO-GO

active-empty pre-branch shape:
  NO-GO

48K fast branch:
  HOLD

active-empty charge elision:
  HOLD
```
...
```text
private/allocators/hakozuna/hakozuna-hz8/current_task.md
private/allocators/hakozuna/hakozuna-hz8/docs/HZ8_V1_LANES.md
private/allocators/hakozuna/hakozuna-hz8/docs/HZ8_MEDIUM_RUN_V1.md
private/allocators/hakozuna/hakozuna-hz8/docs/HZ8_BENCH_GATE.md
```
