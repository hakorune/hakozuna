...
int main(int argc, char** argv) {
  ...
  printf("medium_stats malloc=%zu create=%zu active_reuse=%zu owner_reuse=%zu global_reuse=%zu madvise=%zu owner_scan=%zu owner_steps=%zu global_scan=%zu global_steps=%zu free_lookup=%zu route_lookup=%zu invalid_owned=%zu empty=%zu retain=%zu budget_reject=%zu reactivate=%zu exit_drain=%zu madvise_fail=%zu resident_bytes=%zu resident_peak=%zu madvise_ms=%.3f global_lock_ms=%.3f run_lock_ms=%.3f alloc_slot_ms=%.3f free_slot_ms=%.3f alloc_slot=%zu free_slot=%zu lock_elide_alloc=%zu lock_elide_free=%zu lock_elide_mismatch=%zu free_steps=%zu route_steps=%zu\n",
         ...
         debug.medium_alloc_slot_count, debug.medium_free_slot_count, debug.medium_lock_elide_alloc_candidate, debug.medium_lock_elide_free_candidate, debug.medium_lock_elide_owner_mismatch, debug.medium_free_lookup_step_count, debug.medium_route_lookup_step_count);
  size_t lower_median =
      h8_percentile_size_t(span_lower_bound, (size_t)opt.runs, 0.50);
