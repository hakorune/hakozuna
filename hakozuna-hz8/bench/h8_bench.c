...
int main(int argc, char** argv) {
  ...
  printf("medium_stats malloc=%zu create=%zu active_reuse=%zu owner_reuse=%zu global_reuse=%zu madvise=%zu owner_scan=%zu owner_steps=%zu global_scan=%zu global_steps=%zu free_lookup=%zu route_lookup=%zu invalid_owned=%zu empty=%zu retain=%zu budget_reject=%zu reactivate=%zu exit_drain=%zu madvise_fail=%zu resident_bytes=%zu resident_peak=%zu madvise_ms=%.3f global_lock_ms=%.3f run_lock_ms=%.3f alloc_slot_ms=%.3f free_slot_ms=%.3f alloc_slot=%zu free_slot=%zu lock_elide_alloc=%zu lock_elide_free=%zu lock_elide_mismatch=%zu active_owner_mismatch=%zu owner_list_mismatch=%zu global_skip_foreign=%zu local_free_owner=%zu remote_free_owner=%zu free_steps=%zu route_steps=%zu route_authority_mismatch=%zu remote_pub=%zu remote_lease_ms=%.3f remote_run_lock_ms=%.3f remote_claim=%zu remote_claim_ms=%.3f remote_notify=%zu remote_qpush=%zu remote_qpush_ms=%.3f remote_collect_call=%zu remote_collect_run=%zu remote_collect_slot=%zu remote_collect_ms=%.3f\n",
         ...
         debug.medium_alloc_slot_count, debug.medium_free_slot_count, debug.medium_lock_elide_alloc_candidate, debug.medium_lock_elide_free_candidate, debug.medium_lock_elide_owner_mismatch, debug.medium_active_alloc_owner_mismatch, debug.medium_owner_list_owner_mismatch, debug.medium_global_skip_foreign_attached, debug.medium_local_free_owner_match, debug.medium_remote_free_owner_mismatch, debug.medium_free_lookup_step_count, debug.medium_route_lookup_step_count, debug.medium_route_authority_mismatch, debug.medium_remote_publish_count, (double)debug.medium_remote_owner_lease_ns / 1e6, (double)debug.medium_remote_run_lock_ns / 1e6, debug.medium_remote_pending_claim_count, (double)debug.medium_remote_pending_claim_ns / 1e6, debug.medium_remote_notify_count, debug.medium_remote_queue_push_count, (double)debug.medium_remote_queue_push_ns / 1e6, debug.medium_remote_collect_call_count, debug.medium_remote_collect_run_count, debug.medium_remote_collect_slot_count, (double)debug.medium_remote_collect_ns / 1e6);
