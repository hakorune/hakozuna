...

{
  ...
  for row in "${row_list[@]}"; do
    for kind in "$BASELINE_NAME" "$CANDIDATE_NAME"; do
      ...
      echo
      echo '```text'
      grep -E 'summary|class_map_id|medium_geometry_id|medium_arena_id|medium_residency_id|throughput median|steady_work|post_rss median|peak_rss median|page_faults|interleaved_phase_ms|medium_stats|medium_residual_budget' "$file" || true
      echo '```'
      echo
    done
  done
