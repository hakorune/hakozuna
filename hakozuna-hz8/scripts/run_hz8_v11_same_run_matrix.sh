...
find_lib() {
  ...
  case "${name}" in
    ...
    hz3)
      for p in \
        /mnt/workdisk/public_share/hakozuna_repo/libhakozuna_hz3_scale.so \
        /mnt/workdisk/public_share/hakmem/libhakozuna_hz3_scale.so \
        /mnt/workdisk/public_share/hakmem/allocators/hz3/libhakozuna_hz3_scale_baseline.so; do
        [[ -f "${p}" ]] && { printf '%s\n' "${p}"; return 0; }
      done
      return 1
      ;;
    hz4)
      for p in \
        /mnt/workdisk/public_share/hakozuna_repo/hakozuna-mt/libhakozuna_hz4.so \
        /mnt/workdisk/public_share/hakmem/allocators/libhakozuna_hz4.so \
        /mnt/workdisk/public_share/hakmem/hakozuna/hz4/libhakozuna_hz4.so; do
        [[ -f "${p}" ]] && { printf '%s\n' "${p}"; return 0; }
      done
      return 1
      ;;
    ...
    tcmalloc)
      for p in \
        /mnt/workdisk/public_share/hakmem/allocators/tcmalloc/libtcmalloc_minimal.so \
        /mnt/workdisk/public_share/hakozuna_repo/private/bench-assets/linux/allocators/x86_64/libtcmalloc-minimal4/usr/lib/x86_64-linux-gnu/libtcmalloc_minimal.so.4.5.9; do
        [[ -f "${p}" ]] && { printf '%s\n' "${p}"; return 0; }
      done
      return 1
      ;;
...
allocator_behavior_sha=$(git -C "${ROOT}" rev-parse HEAD)
