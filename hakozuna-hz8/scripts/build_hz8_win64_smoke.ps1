...
& $clang /nologo /D_CRT_SECURE_NO_WARNINGS `
  ...
  /I (Join-Path $root "src") `
  /DH8_ENABLE_DEBUG_STATS `
  /DH8_RECLAIM_ADAPTER_SHADOW_L0 `
  /Fe$outReclaimShadowExe `
  $sources `
  $smoke

if ($LASTEXITCODE -ne 0) {
  throw "reclaim shadow clang-cl failed with exit code $LASTEXITCODE"
}

Write-Host "built $outReclaimShadowExe"

$env:H8_SMOKE_REGULAR_ADOPTION = '0'
& $outReclaimShadowExe
if ($LASTEXITCODE -ne 0) {
  throw "h8_smoke_reclaim_shadow_win failed with exit code $LASTEXITCODE"
}
Remove-Item Env:H8_SMOKE_REGULAR_ADOPTION -ErrorAction SilentlyContinue
