...
OUT_SUFFIX="${OUT_SUFFIX:-hz9_largedirect_probe_gate}"
OUT="$ROOT/bench_results/${STAMP}_${OUT_SUFFIX}"
...
if [[ ! -x "$MATRIX_BIN" ]]; then
  mkdir -p "$(dirname "$MATRIX_BIN")"
  ${CC:-cc} -O2 -g -std=c11 -D_GNU_SOURCE -pthread \
    -o "$MATRIX_BIN" "$ROOT/bench/bench_matrix_malloc.c"
fi
...

{
  ...
  for row in "${row_list[@]}"; do
    for kind in "$BASELINE_NAME" "$CANDIDATE_NAME"; do
      ...
      echo
      echo '```text'
      grep -E 'summary|class_map_id|medium_geometry_id|medium_arena_id|medium_residency_id|throughput median|steady_work|post_rss median|peak_rss median|page_faults|interleaved_phase_ms|direct_large_shadow|direct_large_hotcold|direct_large_sharded_hot|medium_stats|medium_residual_budget' "$file" || true
      echo '```'
      echo
    done
  done
