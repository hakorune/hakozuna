...

{
  ...
  echo "modes: bits=take/free_allocated, cycle_known=single known-slot cycle, known_addr=slot API + address generation, slot_addr=take_slot_addr/free_allocated, fast_addr=take_addr/free_addr_fast, active_addr=active class addr API, table_addr=debug table route, bound_addr=take_addr/free_addr"
  echo '```'
  echo
...
for class_id in 0 1 2 3 4 5; do
  for mode_id in 0 7 2 6 5 4 3 1; do
    mode="bits"
    if [[ "${mode_id}" == "1" ]]; then
      mode="bound_addr"
    elif [[ "${mode_id}" == "2" ]]; then
