...
for class_id in 0 1 2 3 4 5; do
  for mode in direct active active_route active_fast active_route_probe active_range_probe active_exact_probe active_sample8 active_sample64 active_header_probe active_token_probe active_pair_probe active_pair_fast_free active_pair_direct active_pair_fused route2; do
    active_cycle=0
    active_route=0
    route_free=0
    route_interval=64
    if [[ "${mode}" == "active" ]]; then
      ...
      active_route=11
    elif [[ "${mode}" == "active_pair_fused" ]]; then
      active_route=12
