...

{
  ...
  echo "modes: direct=known-slot body, active=active segment known-slot body, active_route=active direct take + route_table_slot free, active_fast=active direct take + active-first route free, active_route_probe=active-first route then known-slot free, route2=table route returns class+slot before free"
  echo '```'
  echo
...
for class_id in 0 1 2 3 4 5; do
  for mode in direct active active_route active_fast active_route_probe route2; do
    active_cycle=0
    active_route=0
    route_free=0
    if [[ "${mode}" == "active" ]]; then
