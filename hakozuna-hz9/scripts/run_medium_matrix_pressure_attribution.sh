...
source "${HZ9_ROOT}/bench/lib/bench_common.sh"

OUTDIR="${OUTDIR:-${HZ9_ROOT}/bench_results/medium_matrix_pressure_attr_$(date -u +%Y%m%dT%H%M%SZ)}"
RUNS="${RUNS:-5}"
THREADS="${THREADS:-16}"
ITERS="${ITERS:-100000}"
LIVE_WINDOW="${LIVE_WINDOW:-4096}"
ALLOCATORS="${ALLOCATORS:-hz9,system}"
TARGET_ALLOCATOR="${TARGET_ALLOCATOR:-hz9}"
MATRIX_BIN="${HZ9_ROOT}/bench/out/bench_matrix_malloc"

mkdir -p "${OUTDIR}" "$(dirname "${MATRIX_BIN}")"

make -C "${HZ9_ROOT}" preload >/dev/null
...
record_target() {
  ...
  start_ns="$(date +%s%N)"
  run_alloc "${TARGET_ALLOCATOR}" "${log}" \
    --runs 1 --threads "${THREADS}" --iters "${ITERS}" \
