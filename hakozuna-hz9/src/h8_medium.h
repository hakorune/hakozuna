...
#else
static inline void* h9_medium_cache_pop(H8ThreadCtx* ctx, uint32_t class_id) {
  (void)ctx;
  (void)class_id;
  return NULL;
}
static inline bool h9_medium_cache_try_push(H8ThreadCtx* ctx, H8MediumRun* run,
                                            void* ptr, size_t slot) {
  (void)ctx;
  (void)run;
  (void)ptr;
  (void)slot;
...
static inline void h9_medium_cache_note_remote_free(H8ThreadCtx* ctx,
                                                    H8MediumRun* run) {
  (void)ctx;
  (void)run;
}
static inline void h9_medium_cache_flush_all(H8ThreadCtx* ctx) {
  (void)ctx;
}
static inline void h9_medium_cache_flush_owner(H8ThreadCtx* ctx,
                                               H8OwnerRecord* owner) {
  (void)ctx;
  (void)owner;
}
static inline void h9_medium_cache_flush_run_locked(H8MediumRun* run) {
  (void)run;
}
#endif
...
#else
static inline void h9_slab_shadow_note_alloc(H8ThreadCtx* ctx,
                                             uint32_t class_id) {
  (void)ctx;
  (void)class_id;
}
static inline void h9_slab_shadow_note_local_free(H8ThreadCtx* ctx,
                                                  H8MediumRun* run,
                                                  size_t slot) {
  (void)ctx;
  (void)run;
...
static inline void h9_slab_shadow_note_remote_escape(H8ThreadCtx* ctx,
                                                     H8MediumRun* run) {
  (void)ctx;
  (void)run;
}
static inline void h9_slab_shadow_flush_all(H8ThreadCtx* ctx) {
  (void)ctx;
}
#endif
#if defined(H8_HZ9_MEDIUM_LOCAL_MAG)
#if defined(H8_MEDIUM_CEILING_ALLOC_NO_SLOT_STATE)
#error "H8_HZ9_MEDIUM_LOCAL_MAG requires slot_state authority"
#endif
#if !defined(H8_HZ9_MEDIUM_LOCAL_MAG_MIN_CLASS_ID)
#define H8_HZ9_MEDIUM_LOCAL_MAG_MIN_CLASS_ID 4u
#endif
typedef enum H8Hz9LocalMagFreeResult {
  H8_HZ9_LOCAL_MAG_FREE_SKIP = 0,
  H8_HZ9_LOCAL_MAG_FREE_OK = 1,
  H8_HZ9_LOCAL_MAG_FREE_INVALID = 2
} H8Hz9LocalMagFreeResult;
static inline bool h8_hz9_local_mag_enabled_run(const H8MediumRun* run) {
  return run && run->class_id < H8_MEDIUM_CLASS_COUNT &&
         run->class_id >= H8_HZ9_MEDIUM_LOCAL_MAG_MIN_CLASS_ID;
}
...
static inline void h8_hz9_local_mag_flush_class(H8ThreadCtx* ctx,
                                                uint32_t class_id,
                                                bool owner_exit) {
  (void)ctx;
  (void)class_id;
...
static inline void h8_hz9_local_mag_flush_owner(H8ThreadCtx* ctx,
                                                H8OwnerRecord* owner,
                                                bool owner_exit) {
  (void)ctx;
  (void)owner;
...
static inline void h8_hz9_local_mag_flush_run_locked(H8MediumRun* run,
                                                     bool owner_exit) {
  (void)run;
  (void)owner_exit;
}
