...
static bool h8_medium_run_free_local_known_slot_timed(H8MediumRun* run,
                                                      size_t slot,
  ...
  if (h8_slot_state_tag(state) != (H8_SLOT_ALLOCATED >> H8_SLOT_TAG_SHIFT) ||
    ...
    if (keep_empty_live &&
        h8_slot_state_tag(state) !=
            (H8_SLOT_ALLOCATED >> H8_SLOT_TAG_SHIFT)) {
      H8_DEBUG_INC(medium_hz9_local_mag_invalid_state);
    }
#endif
#if defined(H8_ENABLE_DEBUG_STATS)
...
#if defined(H8_HZ9_MEDIUM_LOCAL_MAG)
      H8_DEBUG_INC(medium_hz9_local_mag_pending_block);
#endif
    }
    return false;
  }
...
#endif
#if defined(H8_HZ9_MEDIUM_LOCAL_MAG)
  if (keep_empty_live && h8_hz9_local_mag_enabled_run(run)) {
    H8Hz9LocalMagFreeResult mag_res =
        h8_hz9_local_mag_try_free_known(h8_tls_ctx, run, slot, bit);
    if (mag_res == H8_HZ9_LOCAL_MAG_FREE_OK) {
#if defined(H8_ENABLE_DEBUG_STATS)
    ...
    if (mag_res == H8_HZ9_LOCAL_MAG_FREE_INVALID) {
      return false;
    }
  } else if (keep_empty_live) {
...
bool h8_medium_run_free_local_known_slot(H8MediumRun* run, size_t slot,
  ...
  uint64_t start = 0;
#if defined(H8_ENABLE_DEBUG_STATS)
  start = h8_medium_slots_now_ns();
#endif
  return h8_medium_run_free_local_known_slot_timed(run, slot, keep_empty_live,
                                                   start);
}
