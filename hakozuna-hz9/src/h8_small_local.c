...
#if defined(_MSC_VER)
#define H8_NOINLINE __declspec(noinline)
#else
#define H8_NOINLINE __attribute__((noinline))
#endif
...
static H8_NOINLINE void* h8_malloc_non_small_inner(size_t size) {
  if (size <= H8_MEDIUM_MAX_SIZE) {
    ...
    !defined(H9_SLAB_ENTRY_SPLIT_L1) && \
    !defined(H9_SLAB_ALLOC_ROUTE_OFF_PROOF_L1)
#if defined(H9_SLAB_CLASS_COVERAGE_L1)
#if !defined(H9_SLAB_CLASS_MIN_ID)
#define H9_SLAB_CLASS_MIN_ID 2u
#endif
#if H9_SLAB_CLASS_MIN_ID > 0u
    bool slab_candidate = medium_class_id >= H9_SLAB_CLASS_MIN_ID;
#else
    bool slab_candidate = true;
#endif
#else
    bool slab_candidate = size > 49152u;
#endif
    if (slab_candidate) {
      H8ThreadCtx* slab_ctx = h8_thread_ctx_fast();
#if defined(H9_SLAB_REMOTE_ADAPTIVE_L1)
      bool hot = slab_ctx && slab_ctx->owner &&
                 h9_slab_owner_remote_hot(slab_ctx->owner, medium_class_id,
                                          false);
      if (!hot) {
...
#endif
      {
        ...
        void* slab_ptr = h9_slab_alloc(slab_ctx, medium_class_id);
        if (slab_ptr) {
          return slab_ptr;
        }
      }
...
void* h8_malloc_inner(size_t size) {
  ...
  if (size > H8_MAX_SMALL_SIZE) {
    return h8_malloc_non_small_inner(size);
  }
...
void* h8_realloc_inner(void* ptr, size_t size) {
  ...
  if (h8_arena_contains(ptr)) {
    ...
    bool slab_owned = false;
    if (!owned &&
#if defined(H9_SLAB_FREE_ROUTE_OFF_PROOF_L1)
        false &&
#endif
        h9_slab_maybe_contains_hot(ptr)) {
