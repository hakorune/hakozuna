...
typedef enum H9SegmentEntryBenchMode {
  ...
  H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_API = 28,
  H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_PTR_BODY = 29,
  H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_SPLIT_BODY = 30,
  H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_TRUST_BODY = 31,
  H9_SEGMENT_ENTRY_BENCH_TLS_LEDGER = 32,
  H9_SEGMENT_ENTRY_BENCH_TLS_LEDGER_BODY = 33
...
int main(void) {
  ...
  } else if (strcmp(mode, "tlstokencacheapi") == 0) {
    bench_mode = H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_API;
  } else if (strcmp(mode, "tlstokencacheptrbody") == 0) {
    bench_mode = H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_PTR_BODY;
  } else if (strcmp(mode, "tlstokencachesplitbody") == 0) {
    bench_mode = H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_SPLIT_BODY;
  } else if (strcmp(mode, "tlstokencachetrustbody") == 0) {
    bench_mode = H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_TRUST_BODY;
  ...
  if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_API &&
      !h9_segment_entry_debug_acquire_tls_token_cache(class_id)) {
    fprintf(stderr, "segment entry bench failed to acquire tls token cache\n");
    h9_segment_entry_debug_reset();
    return 3;
  }
  H9SegmentEntryTokenCache* tls_token_cache =
      &h9_segment_entry_token_cache_state[class_id];
  uint64_t ok = 0u;
    ...
    if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_LEDGER_BODY) {
      ...
      success =
          h9_segment_entry_debug_cycle_tls_token_cache(class_id, i, touch,
                                                       &ptr);
    } else if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_API) {
      success = h9_segment_entry_debug_cycle_tls_token_cache_body(
          class_id, i, touch, &ptr);
    } else if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_PTR_BODY) {
      success = h9_segment_entry_cycle_token_cache_state_inline(
          tls_token_cache, i, touch, &ptr);
    } else if (bench_mode ==
               H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_SPLIT_BODY) {
      uint32_t slot = UINT32_MAX;
      success =
          h9_segment_entry_token_cache_pop_slot_inline(tls_token_cache, &slot,
                                                       &ptr);
      if (success && touch) {
      ...
      success = success && h9_segment_entry_token_cache_push_slot_inline(
                               tls_token_cache, slot, ptr);
    } else if (bench_mode ==
               H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_TRUST_BODY) {
      uint32_t slot = UINT32_MAX;
      success =
          h9_segment_entry_token_cache_pop_slot_inline(tls_token_cache, &slot,
                                                       &ptr);
      ...
      success = success &&
                h9_segment_entry_token_cache_push_slot_trusted_inline(
                    tls_token_cache, slot, ptr);
  ...
  if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_PTR_BODY &&
      !h9_segment_entry_retire_token_cache_state_inline(tls_token_cache)) {
    fprintf(stderr, "segment entry bench failed to retire tls token cache ptr\n");
    h9_segment_entry_debug_reset();
    return 3;
  }
  if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_SPLIT_BODY &&
      !h9_segment_entry_retire_token_cache_state_inline(tls_token_cache)) {
    fprintf(stderr,
            "segment entry bench failed to retire tls token cache split\n");
    h9_segment_entry_debug_reset();
    return 3;
  }
  if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_TRUST_BODY &&
      !h9_segment_entry_retire_token_cache_state_inline(tls_token_cache)) {
    fprintf(stderr,
            "segment entry bench failed to retire tls token cache trusted\n");
    h9_segment_entry_debug_reset();
    return 3;
  }
  if (bench_mode == H9_SEGMENT_ENTRY_BENCH_TLS_TOKEN_CACHE_API &&
      !h9_segment_entry_debug_retire_tls_token_cache(class_id)) {
    fprintf(stderr, "segment entry bench failed to retire tls token cache api\n");
    h9_segment_entry_debug_reset();
    return 3;
  }
