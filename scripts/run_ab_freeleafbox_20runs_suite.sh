...
# A/B suite for FreeLeafBox: hz3_free PTAG32 leaf optimization
#
# Baseline:  HZ3_FREE_LEAF_ENABLE=0 (original hz3_free)
# Treatment: HZ3_FREE_LEAF_ENABLE=1 (PTAG32 leaf + slow path split)
#
# Usage:
#   RUNS=20 ./hakozuna/hz3/scripts/run_ab_freeleafbox_20runs_suite.sh
#   RUNS=10 THREADS=16 ./hakozuna/hz3/scripts/run_ab_freeleafbox_20runs_suite.sh
...
BASE_SO="/tmp/hz3_freeleaf_base.so"
TREAT_SO="/tmp/hz3_freeleaf_treat.so"

# Baseline: FreeLeafBox disabled
BASE_DEFS_EXTRA="${BASE_DEFS_EXTRA:--DHZ3_FREE_LEAF_ENABLE=0}"

# Treatment: FreeLeafBox enabled (default when all prereqs met)
TREAT_DEFS_EXTRA="${TREAT_DEFS_EXTRA:-}"

build_lib() {
  local out_so="$1"; shift
  local defs_extra="$1"; shift
  make -C hakozuna/hz3 clean all_ldpreload_scale -j HZ3_LDPRELOAD_DEFS_EXTRA="${defs_extra}" >/tmp/build_hz3_freeleaf_ab.log 2>&1
...
echo "[FREELEAF_AB] RUNS=${RUNS} THREADS=${THREADS} WS=${WS} RING_SLOTS=${RING_SLOTS}"
echo "[FREELEAF_AB] BASE_DEFS_EXTRA='${BASE_DEFS_EXTRA}'"
echo "[FREELEAF_AB] TREAT_DEFS_EXTRA='${TREAT_DEFS_EXTRA}'"

# Baseline = FreeLeafBox disabled
echo "[FREELEAF_AB] Building baseline (LEAF=0)..."
build_lib "${BASE_SO}" "${BASE_DEFS_EXTRA}"

# Treatment = FreeLeafBox enabled (default)
echo "[FREELEAF_AB] Building treatment (LEAF=1)..."
...
echo ""
echo "[FREELEAF_AB] Done. Check /tmp/ab_*_hz3_freeleaf_*.log for individual run logs."
